...
| `wireclient` | `cimmeria-wireclient` | **Headless test client.** Drives the SOAP auth leg and builds and parses the Mercury phase-3 handshake, and loads JSONL session traces with a diff policy. It has no UDP socket or replay engine yet, so it cannot replay captured `.pcap` sessions end to end. Pairs with `tools/pcap_to_session.py` (JSONL exporter built atop `tools/pcap_dissect.py`). See [docs/architecture/wireclient.md](../docs/architecture/wireclient.md). |
