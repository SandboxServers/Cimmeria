...
| `occluder` | `cimmeria-occluder` | Collision-geometry occluder for server-side line of sight (NA27, [#784](https://github.com/SandboxServers/Cimmeria/issues/784)). It holds the `.occ` format (a tiled column grid of solid Y spans with sub-cell rectangles, plus an exact terrain heightfield, zlib per layer), the builder that rasterises triangles into it, and the eye-to-eye segment test. It is shared by `navmesh-extractor`'s `occluder_extract` so the writer and reader cannot drift. No world ships a `.occ` yet: the phase-1 sizes are over budget, see [the NA27 worknote](../docs/analysis/npc-ai-restoration/worknotes/na27-occluder-phase1.md). |
