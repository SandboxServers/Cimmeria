...
| `server-harness` | `cimmeria-server-harness` | Spawn/readiness/reap harness for the `cimmeria-server` process, for integration tests that need a real server. Reserves all six ports from the OS, gates on an observed readiness condition (`GET /api/config/status`) rather than a sleep, and guarantees reaping via `Drop` plus a kernel-level kill-on-parent-death layer. Resolves risk 1 of the [wireclient ADR](../docs/architecture/wireclient.md). See [docs/architecture/session-bootstrap.md](../docs/architecture/session-bootstrap.md). |
