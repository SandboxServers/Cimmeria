...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Wait for Tauri context with timeout
  let attempts = 0;
  while (!window.__TAURI__ && attempts < 500) {
    await new Promise(resolve => setTimeout(resolve, 10));
    attempts++;
  }
  if (!window.__TAURI__) {
    console.error('Tauri context failed to load after 5 seconds');
    document.getElementById('error-message').textContent = 'Failed to initialize Tauri context';
  ...
  async function init() {
    try {
      ...
      console.log('Install path set to:', installPathInput.value);

      const state = await invoke('cmd_check_installation');
      console.log('Installation state:', state);
      if (state === 'Installed') {
        showState('ready');
      } else {
        showState('not-installed');
