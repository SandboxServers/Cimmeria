...
mod tests {
    ...
    /// function must not panic, must not emit a wire packet, and must leave
    /// every other character row alone (asserted via a sentinel sibling).
    #[tokio::test]
    async fn drops_grant_silently_when_player_row_missing() {
        let pool = require_db_or_skip!();
