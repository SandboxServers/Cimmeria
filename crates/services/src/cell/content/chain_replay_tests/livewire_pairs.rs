...
//!
//! The last test pins the new `difficulty` param (defect B5) against a
//! sentinel chain: no seeded row sets it today, so a real chain cannot
//! prove the value survives loader → `Action` → `CellToBaseMsg`.
...
/// Sentinel `content_chains.chain_id` for the `difficulty` round-trip.
/// Fits in `i32`. Sibling reservations in `crates/services` run
/// `0x7000_1000..0x7000_1B00`, `0x7000_2000`, `0x7000_3000`, `0x7000_4000`,
/// `0x7000_4242` and `0x7000_5000` (`grant_xp`); this steps past all of
/// them. Cleanup deletes the exact ids inserted, never a range.
const DIFFICULTY_CHAIN_ID: i32 = 0x7000_6000;
const DIFFICULTY_TAG: &str = "CIMMERIA_TEST_MINIGAME_DIFFICULTY_TAG";
/// Neither the loader default (1) nor a boundary — a hardcoded default or
/// a clamp-to-range bug would not reproduce it.
...
async fn execute_and_drain_starts(
...
// ── difficulty round-trip (defect B5) ────────────────────────────────────
