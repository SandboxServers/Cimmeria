//! Bandolier active-slot operations: dirty-ammo persistence flush and the
//! active-slot swap (including the weapon→weapon holster choreography and
//! the per-weapon ability grant). These handlers interleave reads and
//! mutations on the cell entity, so each carefully scopes its `&mut`
//! borrows around await points.
