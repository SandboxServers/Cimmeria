...
//! - [`detection`] — the player-side proximity sweep behind the
//!   `OnPlayerEnteredCover` / `OnPlayerLeftCover` content triggers.
//! - [`coverage`] — the per-space "how much of the cover sits on the navmesh"
//!   summary logged when a space loads.
//!
...
pub use ai_integration::{
    horizontal, lock_or_recover, maintain_cover_for_npc, maintain_cover_for_npc_checked,
    maintain_cover_for_npc_traced, CoverDecision, CoverQuery, CoverTrace, NoCoverReason,
    ReleaseReason, COVER_ARRIVE_RADIUS, COVER_BLIND_GRACE, COVER_REPICK_COOLDOWN,
    IN_RANGE_MAX_MOVE, PICK_RANGE_MARGIN, SEEK_RETRY,
};
