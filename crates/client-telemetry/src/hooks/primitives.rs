...
//! inline hooks to MinHook. The future client-patch crates
//! (#434 Phases 1-3) need the *same* page-protect / pointer-rewrite /
...
mod tests {
    ...
    fn inline_hook_detours_and_chains_to_original() {
        ...
        let hook =
            unsafe { install_inline_hook(target, detour) }.expect("inline hook should install");
    ...
    fn replace_iat_slot_swaps_and_returns_original() {
        ...
        let original = unsafe { replace_iat_slot(slot_addr, iat_detour as *const () as usize) }
            .expect("iat swap");
