...
| 1.4 — BSP `Model` / `Polys` decoder | follow-up — **on the critical path for Castle**, see below. Ghidra trace **done**, layout byte-exact validated against real `Castle-000a0002.umap` data; recipe in `docs/reverse-engineering/findings/bsp-model-polys-serialize.md`. `Polys` confirmed NOT stripped in cooked packages. Implementation (`model.rs` in `crates/upk-objects`) not yet written |
...
- **`Model` / `Polys` BSP decoder** — layout confidence now HIGH (byte-exact
  ...
  See "Interior floors are BSP" above: for Castle this is the blocking
