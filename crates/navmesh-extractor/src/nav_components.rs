...
mod tests {
    ...
    /// Build a synthetic `XrcNav` from explicit world-space quads.
    ///
    /// `quads` are `[(x, z)]` corner pairs at a fixed height; adjacency is
    /// supplied explicitly so the test exercises the decode path rather
    /// than a re-derivation of what we are trying to test.
    struct NavBuilder {
        cs: f32,
        ch: f32,
        verts: Vec<[u16; 3]>,
        polys: Vec<[u16; 8]>, // nvp = 4 → 4 verts + 4 neighbours
    }
    ...
    impl NavBuilder {
        ...
        fn vert(&mut self, x: u16, y: u16, z: u16) -> u16 {
            self.verts.push([x, y, z]);
            (self.verts.len() - 1) as u16
        }

        /// Axis-aligned unit quad with its lower corner at grid `(x, z)`,
        /// at grid height `y`.
        fn quad(&mut self, x: u16, y: u16, z: u16) -> u16 {
            let a = self.vert(x, y, z);
            let b = self.vert(x + 1, y, z);
            let c = self.vert(x + 1, y, z + 1);
            let d = self.vert(x, y, z + 1);
            self.polys
                .push([a, b, c, d, 0xffff, 0xffff, 0xffff, 0xffff]);
