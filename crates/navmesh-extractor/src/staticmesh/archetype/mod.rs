...
/// What [`resolve_actor_archetype`] concluded.
///
/// The three cases are deliberately distinct. `NotInstanced` and
/// `Resolved(default)` both mean "inherit nothing", but
/// `Unresolved` does **not**: it means the chain that would have told
/// us whether the actor collides could not be read, and treating that
/// as "inherit nothing" silently applies UE3's `bCollideActors = true`
/// default to a template that may have said `false`.
#[derive(Debug, Clone, Copy, PartialEq)]
pub enum ActorArchetypeResolution {
    /// `Archetype == 0`: the instance's own properties are the whole
    /// story, which is not an assumption but a fact about the export.
    NotInstanced,
