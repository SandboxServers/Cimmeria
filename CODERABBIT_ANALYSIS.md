...
## 🟡 IMPORT & MODULE PATH ERRORS (10 issue-types across 9 file-occurrences) — FIX BEFORE BUILD

> Counting note: the breakdown below mixes issue-types (e.g., "naming inconsistency")
> with per-file occurrences. The "8 issues" figure used in earlier revisions of this
> doc was inconsistent with the table.
...
1. Fix bandolier state loss on world entry (queries + apply to entity)
2. Fix bandolier fields ignored in `InitPlayerState` (apply to cell entity)
3. Fix gate travel returning empty stargates
4. Fix inventory move silent 0-rows commit (row-affected check)
5. Verify buyback container cannot leave orphaned `stack_size=0` rows
6. Update test assertion that still expects the removed `EntityMethodCall` flow
...
> **Snapshot:** This recommendation reflects the CodeRabbit review state as of
> commit `68d20a3` (the initial review pass on PR #77). Subsequent commits on
> the `method-refactoring` branch fold in additional CodeRabbit rounds — see
> the PR history for the current resolved/unresolved set.
