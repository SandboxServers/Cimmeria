...
class QtDatabaseIntPropertyManager : public QtIntPropertyManager
{
    Q_OBJECT
public:
    QtDatabaseIntPropertyManager(QObject *parent = 0);
    ~QtDatabaseIntPropertyManager();

    QString getDbName(const QtProperty *property) const;

public Q_SLOTS:
    void setDbName(const QString &property, QString const & name, QtProperty *invalidate = nullptr);
    void flushNames();

Q_SIGNALS:
    void dbNameChanged(QtProperty *property, QString const & name);
...
private:
    mutable QVector<QtProperty const *> pendingNames_;
    QMap<QString, QString> dbNames_;
    Q_DISABLE_COPY(QtDatabaseIntPropertyManager)
};
...
public Q_SLOTS:
    void setValue(QtProperty *property, qlonglong val);
    void setEnumNames(QtProperty *property, const QStringList &names);
    void setEnumIcons(QtProperty *property, const QMap<int, QIcon> &icons);
Q_SIGNALS:
    void valueChanged(QtProperty *property, qlonglong val);
