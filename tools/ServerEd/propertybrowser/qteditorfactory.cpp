...
QtProperty * QtSpinBoxFactory::propertyFromWidget(QSpinBox * widget)
{
    auto it = d_ptr->m_editorToProperty.find(widget);
    if (it != d_ptr->m_editorToProperty.end())
        return it.value();
    else
        return nullptr;
}

QSpinBox * QtSpinBoxFactory::widgetFromProperty(QtProperty * property)
{
    auto it = d_ptr->m_createdEditors.find(property);
    if (it != d_ptr->m_createdEditors.end() && it.value().size() == 1)
        return it.value()[0];
    else
        return nullptr;
}
...
QtDatabaseWrapperWidget::QtDatabaseWrapperWidget(QWidget *parent)
    : QWidget(parent), button_(nullptr), label_(nullptr), editBox_(nullptr)
{
}

QtDatabaseWrapperWidget::~QtDatabaseWrapperWidget()
{
}

void QtDatabaseWrapperWidget::resizeEvent(QResizeEvent * event)
{
    QWidget::resizeEvent(event);
    if (button_)
    {
        button_->setFixedSize(30, height());
        button_->move(event->size().width() - button_->size().width(), 0);
    }
    if (label_)
    {
        label_->setFixedSize(qMax(event->size().width() - 115, 0), height());
        label_->move(85, 0);
    }
    if (editBox_)
    {
        editBox_->setFixedSize(80, height());
    }
}

void QtDatabaseWrapperWidget::setPushButton(QWidget * button)
{
    button_ = button;
}

void QtDatabaseWrapperWidget::setLabel(QLabel * label)
{
    label_ = label;
}

void QtDatabaseWrapperWidget::setEditBox(QWidget * editBox)
{
    editBox_ = editBox;
}
...
/*!
    Creates a factory with the given \a parent.
*/
QtDatabaseSpinBoxFactory::QtDatabaseSpinBoxFactory(QObject *parent)
...
/*!
    Destroys this factory, and all the widgets it has created.
*/
QtDatabaseSpinBoxFactory::~QtDatabaseSpinBoxFactory()
{
}
...
/*!
    ...
    Reimplemented from the QtSpinBoxFactory class.
*/
QWidget *QtDatabaseSpinBoxFactory::createEditor(QtIntPropertyManager *manager, QtProperty *property,
        QWidget *parent)
{
    QtDatabaseWrapperWidget * wrapper = new QtDatabaseWrapperWidget(parent);
    QSpinBox * editBox = dynamic_cast<QSpinBox *>(QtSpinBoxFactory::createEditor(manager, property, wrapper));
    Q_ASSERT(editBox);

    QLabel * label = new QLabel(wrapper);
    label->setFixedSize(qMax(wrapper->width() - 115, 0), wrapper->height());
    QString name = static_cast<QtDatabaseIntPropertyManager *>(manager)->getDbName(property);
    label->setText(name);
    label->move(85, 0);

    QPushButton * browserBtn = new QPushButton(wrapper);
    browserBtn->setFixedSize(30, wrapper->height());
    browserBtn->move(wrapper->size().width() - browserBtn->size().width(), 0);
    browserBtn->setText("...");
    QObject::connect(browserBtn, SIGNAL(clicked()), this, SLOT(onBrowseBtnClicked()));
    QObject::connect(editBox, SIGNAL(valueChanged(int)), SLOT(onValueUpdated(int)));

    wrapper->setPushButton(browserBtn);
    wrapper->setLabel(label);
    wrapper->setEditBox(editBox);
    return wrapper;
}

void QtDatabaseSpinBoxFactory::onBrowseBtnClicked()
{
    emit onBrowseClicked();
}

void QtDatabaseSpinBoxFactory::onValueUpdated(int)
{
    emit valueUpdated();
}

void QtDatabaseSpinBoxFactory::dbNameUpdated(QtProperty * property, QString name)
{
    QWidget * widget = widgetFromProperty(property);
    if (widget)
    {
        QLabel * label = widget->parentWidget()->findChild<QLabel *>();
        if (label)
        {
            label->setText(name);
        }
...
public:
    ...
    void slotPropertyChanged(QtProperty *property, qlonglong value);
    void slotEnumNamesChanged(QtProperty *property, const QStringList &);
    void slotEnumIconsChanged(QtProperty *property, const QMap<int, QIcon> &);
    void slotSetValueCompat(int value);
    void slotSetValue(qlonglong value);
};

void QtEnumEditorFactoryPrivate::slotPropertyChanged(QtProperty *property, qlonglong value)
{
    if (!m_createdEditors.contains(property))
        return;
...
void QtEnumEditorFactoryPrivate::slotSetValueCompat(int value)
{
    slotSetValue(value);
}

void QtEnumEditorFactoryPrivate::slotSetValue(qlonglong value)
{
    QObject *object = q_ptr->sender();
...
{
    connect(manager, SIGNAL(valueChanged(QtProperty*,qlonglong)),
                this, SLOT(slotPropertyChanged(QtProperty*,qlonglong)));
        ...
        QWidget *parent)
{
    ...
    connect(editor, SIGNAL(currentIndexChanged(int)), this, SLOT(slotSetValueCompat(int)));
    connect(editor, SIGNAL(destroyed(QObject*)),
                this, SLOT(slotEditorDestroyed(QObject*)));
    return editor;
}
...
{
    disconnect(manager, SIGNAL(valueChanged(QtProperty*,qlonglong)),
                this, SLOT(slotPropertyChanged(QtProperty*,qlonglong)));
