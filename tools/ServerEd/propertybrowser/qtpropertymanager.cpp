...
QtDatabaseIntPropertyManager::QtDatabaseIntPropertyManager(QObject *parent)
    : QtIntPropertyManager(parent)
{
}

QtDatabaseIntPropertyManager::~QtDatabaseIntPropertyManager()
{
}

QString QtDatabaseIntPropertyManager::getDbName(QtProperty const * property) const
{
    auto const it = dbNames_.find(property->propertyName());
    if (it != dbNames_.end())
        return it.value();
    else
    {
        if (!pendingNames_.contains(property))
        {
            pendingNames_.push_back(property);
            emit const_cast<QtDatabaseIntPropertyManager *>(this)->dbNameRequested(const_cast<QtProperty *>(property));
        }
        return "Unknown";
    }
}

void QtDatabaseIntPropertyManager::setDbName(QString const & property, QString const & name, QtProperty *invalidate)
{
    dbNames_[property] = name;
    if (invalidate)
    {
        qDebug("Sending valueChanged(%d)!", value(invalidate));
        emit propertyChanged(invalidate);
        // emit valueChanged(invalidate, value(invalidate));
    }
}

void QtDatabaseIntPropertyManager::flushNames()
{
    dbNames_.clear();
}

void QtDatabaseIntPropertyManager::uninitializeProperty(QtProperty * property)
{
    QtIntPropertyManager::uninitializeProperty(property);
    auto it = dbNames_.find(property->propertyName());
    if (it != dbNames_.end())
        dbNames_.erase(it);
}

QString QtDatabaseIntPropertyManager::valueText(const QtProperty *property) const
{
    QString value = QtIntPropertyManager::valueText(property);
    return value + " (" + getDbName(property) + ")";
}
...
{
    return getValue<qlonglong>(d_ptr->m_values, property, -1);
}
...
/*!
    ...
    \sa value(), valueChanged()
*/
void QtEnumPropertyManager::setValue(QtProperty *property, qlonglong val)
{
...
{
    return getValue<qlonglong>(d_ptr->m_values, property, 0);
}
...
/*!
    ...
    \sa value(), valueChanged()
*/
void QtFlagPropertyManager::setValue(QtProperty *property, qlonglong val)
{
    ...
    if (val > ((qlonglong)1 << data.flagNames.count()) - 1)
        return;
    ...
    while (itProp.hasNext()) {
        ...
        if (prop)
            d_ptr->m_boolPropertyManager->setValue(prop, val & ((qlonglong)1 << level));
        level++;
    }
