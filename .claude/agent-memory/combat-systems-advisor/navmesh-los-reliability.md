...
- There is no fire-time LoS check in `use_ability`. The client has `CONDITION_FEEDBACK_NoLOS = 40` and the GM `toggleCombatLOS`, so the original server probably checked. Add the check for players only once a geometry occluder exists, or players will get false "no LoS" errors behind desks.
