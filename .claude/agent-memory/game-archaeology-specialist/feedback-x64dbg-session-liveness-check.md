...
## Toolchain upgrade (2026-07-25)

x64dbg was upgraded mid-investigation: plugin `v0.8.0-ghost_fungus`, client `0.9.0`. The prior live SGW.exe client process was terminated as part of the upgrade, so the next live session starts from a fresh launch — there is nothing to reconnect to. The upgrade adds **programmatic log capture**, which should substantially improve the non-freezing-breakpoint workflow this file and [[feedback_x64dbg_nonfreezing_breakpoints]] describe (previously: hit-count polling via repeated `list_breakpoints` calls; now: direct log retrieval). Prefer the new log-capture path over BP-hit-counting in future sessions once its exact tool surface is confirmed live.
