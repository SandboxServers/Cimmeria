...
- `event_id = 6` (`EVENT_ITEM_MELEE`) and `event_id = 7` (`EVENT_ITEM_RANGED`) — **fully live.** Loaded at cell startup into `space_mgr.item_event_set_abilities: HashMap<(item_id,event_id), ability_id>` by `load_item_event_set_abilities` (`crates/cell-catalog/src/cell/spawner/abilities.rs:317`). Consulted by:
