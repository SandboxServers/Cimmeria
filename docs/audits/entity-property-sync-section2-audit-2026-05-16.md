...
spec_version: docs/drafts/spec/entity-property-sync.md @ commit 70a61cc (branch worktree-bible+spec-entity-property-sync)
status: complete
...
revision_history:
  ...
  - 2026-05-16 v5 — Target 1 inline-corrected from "9 entries" to "16 entries (9 primary + 7 deprecated aliases)" + follow-on-question paragraph re-pointed to Appendix A's resolution; Appendix C.5 carries a methodology-correction note explaining the V4 envelope-vs-prefix conflation (CR-7 + CP-14, PR #305 round 2)
  - 2026-05-17 v6 — Appendix C.3 typeID re-decode: 0-based document-index assignment confirmed by `EntityDescription_ReadFromStream @ SGW.exe@0x01590520` (called from `BWClassMap::InitEntityClassTable @ SGW.exe@0x00c67190`). Earlier 1-based interpretation (typeId=2 → SGWBeing, typeId=7 → SGWDuelMarker) was inherited from `tools/entity_property_sync_resolver.py`'s hard-coded `[expect 0x0003 = SGWPlayer]` annotation and was circular. Re-decode: typeId=2 = SGWPlayer (correct world-entry payload), typeId=7 = **Account** (correct character-select payload). *Superseded 2026-09-19:* this entry originally read typeId=7 as SGWBlackMarket and called `ACCOUNT_CLASS_ID = 0x07` a Rust-server bug (issue #313); that reading used the raw document index, but the wire typeID is the clientIndex, which skips `<ServerOnly/>` entries — see Appendix C.3. Also closes F.2 / F.2.1 / F.3 / Target 4 OQ-2 disposition reconciliation (all now uniformly RESOLVED per F.2.2's tautological-gate finding). Resolver tool's `scan_body_for_sub_slots` made direction-aware (CR-9 follow-on)
...
**Final disposition: RESOLVED — see Appendix F.2.2 below. This Target 4 section preserves the original first-pass investigation, whose conclusions about "two distinct DataDescription layout forms" and a "write only if internal name == external name" gate were later superseded. F.2.2 traces the actual write sites and proves both fields are always zero-length, so the gate is tautological (vestigial BigWorld dead code). The recommendation at the end of this section is also superseded — see F.3's superseded-guidance note and F.2.2's final findings for the current chapter direction.**

**Original disposition (this section): RESOLVED (partial)**
...
**Re-decoded (binary anchor).** Under the **0-based** typeID assignment confirmed by `EntityDescription_ReadFromStream @ ghidra://SGW.exe@0x01590520` (called from `BWClassMap::InitEntityClassTable @ ghidra://SGW.exe@0x00c67190` to load `entities/entities.xml`): typeId=2 = **SGWPlayer** (the actual world-entry payload — msg 2), and typeId=7 = **Account** (the character-select payload — msgs 1 and 3). These captures were emitted by the Rust server, not the legacy SGW server, and both emissions are correct.

**Superseded reading (corrected 2026-09-19).** This appendix previously decoded typeId=7 as `SGWBlackMarket` and reported a Rust-server bug (`ACCOUNT_CLASS_ID = 0x07`, "should be `0x08`" — [issue #313](https://github.com/SandboxServers/Cimmeria/issues/313)). That decode applied the *raw* `entities.xml` document index. `EntityDescriptionMap_parse @ ghidra://SGW.exe@0x01590520` keeps a second counter — the clientIndex at `desc+0x1e`, advanced only when `name_ == clientName_` — and `EntityDescription_parse @ ghidra://SGW.exe@0x01593cd0` blanks `clientName_` for `<ServerOnly/>` defs, so `SGWBlackMarket` (raw index 7) takes no clientIndex and `Account` (raw index 8) is clientIndex **7**. The client resolves wire typeIDs through the clientIndex → raw-index map (`FUN_0158e710 @ ghidra://SGW.exe@0x0158e710`). The "character-list UI doesn't need a live Account entity" argument was also wrong: `createCharacter` / `playCharacter` are exposed Account base methods that the client can only send from an instantiated Account entity. Issue #313 was closed as invalid; the canonical statement is `docs/protocol/client-verified-wire-formats.md` "Entity Class IDs".

**Earlier interpretation (now superseded).** This appendix originally read typeId=2 as `SGWBeing` and typeId=7 as `SGWDuelMarker` under a 1-based document-index assumption inherited from the resolver tool's `[expect 0x0003 = SGWPlayer]` constant (`tools/entity_property_sync_resolver.py:446`). That assumption was never anchored to Ghidra evidence; the chain was circular. The decompile of `EntityDescription_ReadFromStream` settles it: `uVar18 = 0` initial, `(short)uVar18` written as typeID, `uVar18 + 1` per slot regardless of parse success. **Layout (entityId u32 + typeId u16 + property stream) is still confirmed** — only the typeID-to-entity-name resolution was wrong. The msg 2 (`02 00 00 00 02 00`) is now correctly identified as a real SGWPlayer wire-format witness (with empty property stream as expected from §1.2's keyword surface, since SGW `.def`s set no `OWN_CLIENT`/`OTHER_CLIENT` bits), so the "non-empty property stream case is not yet wire-witnessed" note continues to stand for a different reason (no `OWN_CLIENT`/`OTHER_CLIENT` keywords exist in any SGW `.def`, not "session ended before SGWPlayer emitted").
...
**Final disposition: RESOLVED. See F.2.2 below for the closure (the comparison gate is tautological — both fields are always zero-length). This F.2 section preserves the historical investigation log that established only a partial resolution; F.2.1 then ruled out `FUN_0158f260` as the runtime write site, and F.2.2 closed the question by tracing the actual write sites and proving the gate is dead. References elsewhere in this audit (F.0 summary table, F.3 chapter guidance below) should be read against F.2.2's final disposition, not F.2's partial state.**

**Disposition at the time this section was written: PARTIALLY-RESOLVED — the "two name fields" premise is corrected. The `0x110`-byte parse-time `DataDescription` has ONE name field; `+0x24` in `DataDescription_ParseFlags` is a `SmartPointer<DataSection>` (the Default child section), not a second name string.**
...
**Revised disposition for OQ-2 (this section)**: RESOLVED — but the *interpretation* below is partially superseded by F.2.2. `FUN_0158f260` is correctly ruled out as the write site for the `StdStringMSVC` fields; that part of this finding stands. The trailing claim that "the comparison in `EntityDescription_FindAndWritePropertyByName` is a name-consistency gate between two name variants of the same property" is **SUPERSEDED by F.2.2**: F.2.2 traces every write site in the SGW parse chain and shows that both `+0x24` and `+0x40` are always zero-length `StdStringMSVC` fields, so the comparison is tautological (dead vestigial BigWorld infrastructure), not a name-variant gate. The hypothesis that the "two distinct DataDescription layout forms" exist at runtime is also superseded — there is only the parse-time form, with the runtime view interpreting `+0x24`'s SmartPointer storage as an (empty) inline-SSO string when read as `StdStringMSVC`.
...
**§1.15 OQ-2 (dual name fields) — SUPERSEDED guidance.** This paragraph's chapter-update instruction (update to PARTIALLY-RESOLVED, recommend the doc-writer add a "two layout forms" note) was issued before F.2.2 closed the question. The **current** chapter guidance is: update §1.15 OQ-2 to **RESOLVED** per F.2.2 below — the `+0x24` SmartPointer write is the only write through `DataDescription_ParseFlags`, the `StdStringMSVC` length fields at `+0x34`/`+0x50` are never updated past the constructor's `length=0` default, and the `EntityDescription_FindAndWritePropertyByName` comparison is a vestigial BigWorld dead check (both fields always empty, gate always passes). There is no second runtime layout form; the original "two distinct DataDescription layout forms" inference is a structural hypothesis that F.2.2 ruled out by tracing actual write sites. The follow-up to decompile `FUN_0158f260` has also been completed (F.2.1) and rules it out as a write site. The Ghidra constructor annotation "type name" for `+0x24` is still likely wrong — the field is structurally a `SmartPointer<DataSection>` storage slot, not a string — and that observation stands.
