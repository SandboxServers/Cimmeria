...
**Re-decoded (binary anchor).** Under the **0-based** typeID assignment confirmed by `EntityDescription_ReadFromStream @ ghidra://SGW.exe@0x01590520` (called from `BWClassMap::InitEntityClassTable @ ghidra://SGW.exe@0x00c67190` to load `entities/entities.xml`): typeId=2 = **SGWPlayer** (the actual world-entry payload — msg 2), and typeId=7 = **SGWBlackMarket** (`<ServerOnly/>` in `SGWBlackMarket.def`). These captures were emitted by the Rust server, not the legacy SGW server; the two typeId=7 emissions reflect a Rust-server bug at `crates/services/src/mercury/mod.rs:104` where `ACCOUNT_CLASS_ID = 0x07` (should be `0x08` per the 0-based table — Account is the 9th XML entry, position 8). The client silently fails to instantiate SGWBlackMarket per chapter §1.16 F4 (no in-handler validation gate; server-only entities have no client-loaded `.def`); the character-list UI works through the subsequent `BASEMSG_ON_CHARACTER_LIST` (msg 0x82) which is a non-entity message and doesn't depend on a live Account entity. Bug tracked as [issue #313](https://github.com/SandboxServers/Cimmeria/issues/313).
