...
| 3 | **Critical → CLOSED in #389** | §2.5.2 + §2.5.1 | Client→server msg `0x0B` `restoreClientAck` parsed as `WORD_LENGTH`; spec says `CONSTANT_LENGTH = 4` (no length prefix; literal `i32 = 0` payload). The `WORD_LENGTH` parser reads 2 bytes as a u16 length-prefix that doesn't exist on the wire, then advances `2 + that_length` bytes — for a real 4-byte zero body, it reads `0x0000 = length 0` and leaves the trailing 2 bytes unconsumed for the next message. Bundle parser desyncs by 2 bytes. **Fixed:** dispatch now reads `read_constant_payload(body, offset, 4)`; three regression guards pin the framing (`crates/services/src/base/connect_loop/encrypted.rs::tests`). | `crates/services/src/base/connect_loop/encrypted.rs:141-142` |
...
| `0x0B` | restoreClientAck | CONSTANT_LENGTH=4 literal `i32=0` | `crates/services/src/base/connect_loop/encrypted.rs::read_client_message_payload` `0x0B` arm: `read_constant_payload(body, offset, 4)` | none | CONFORMS — finding #3 closed by #389 (was DIVERGES (Critical) at audit time) |
...
| **S11.2** restoreClientAck (0x0B) body is literal `i32=0`; do not parse as entity_id | Server must accept 4 bytes as status code, NOT entity_id | CONFORMS on the wire-framing side after #389 (finding #3 closed); no semantic handler exists yet (M8 follow-up) |
...
| 3 | **Critical → CLOSED in #389** | §2.5.2 + §2.4 wire shape for restoreClientAck | `crates/services/src/base/connect_loop/encrypted.rs:141-142` | msg `0x0B` parsed as `WORD_LENGTH` (u16 prefix); spec says `CONSTANT_LENGTH = 4` (no prefix). Real client packet desyncs the bundle parser by 2 bytes. **Fixed:** `read_client_message_payload` extracted + `0x0B` arm now `CONSTANT_LENGTH=4`; 3 regression guards in `encrypted.rs::tests` |
...
| M7 | §2.4 R10 — 15-second silence-tolerance gate | Reconcile three inactivity-timeout constants (`crates/mercury/src/lib.rs:55` 300s; `crates/services/src/base/tick_sync.rs:32` 60s; spec 15s) into a coherent semantic. Recommend: name `MERCURY_PEER_DEAD_MS` for the bookkeeping value (current 300s) and add `UE3_INACTIVITY_TIMEOUT_MS = 15_000` for the wire-observable R10 silence-tolerance edge. ✅ **RESOLVED** (see #293): the constants are split exactly as recommended (`MERCURY_PEER_DEAD_MS = 300_000`, `UE3_INACTIVITY_TIMEOUT_MS = 15_000`), and `tick_sync.rs` now uses the 15 s UE3 value instead of a bespoke 60 s |
...
| N3 | §2.5.1 — `requestEntityUpdate` (msg `0x07`) handler | ✅ **RESOLVED** in [PR #390](https://github.com/SandboxServers/Cimmeria/pull/390). Dispatch arm at [`crates/services/src/base/connect_loop/encrypted.rs`](../../crates/services/src/base/connect_loop/encrypted/mod.rs) (search `REQUEST_ENTITY_UPDATE`) parses `[u32 header][N × u32 entity_id]` and forwards to `BaseToCellMsg::RequestEntityUpdate`. Cell handler at [`crates/services/src/cell/service/base_messages/request_entity_update.rs`](../../crates/services/src/cell/service/base_messages/request_entity_update.rs) re-emits a synthetic `EnteredAoI` for each requested entity that is currently in the witness's AoI (out-of-AoI requests are dropped — anti-probe; payload capped at 64 ids per request). |
