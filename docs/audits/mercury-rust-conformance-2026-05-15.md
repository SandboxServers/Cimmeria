...
| N3 | §2.5.1 — `requestEntityUpdate` (msg `0x07`) handler | ✅ **RESOLVED** in [PR for issue #289](https://github.com/SandboxServers/Cimmeria/issues/289). Dispatch arm at [`crates/services/src/base/connect_loop/encrypted.rs`](../../crates/services/src/base/connect_loop/encrypted.rs) (search `REQUEST_ENTITY_UPDATE`) parses `[u32 header][N × u32 entity_id]` and forwards to `BaseToCellMsg::RequestEntityUpdate`. Cell handler at [`crates/services/src/cell/service/base_messages/request_entity_update.rs`](../../crates/services/src/cell/service/base_messages/request_entity_update.rs) re-emits a synthetic `EnteredAoI` for each requested entity that is currently in the witness's AoI (out-of-AoI requests are dropped — anti-probe). |
