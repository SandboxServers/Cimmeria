...
| Filter `enabled = false` chains in the loader (`WHERE enabled = true` in [engine_loader.rs:51](../../crates/services/src/cell/content/engine_loader.rs#L51)) | Confirmed: the boot `SELECT` reads the `enabled` column but has no `WHERE` clause on it, so a chain flagged `enabled = false` in the seed is still registered and still fires |
