...
**Status:** Done — merged to `main` in PR #646 (2026-09-17). Region8 aggro landed as chain 1008. **Scope title:** Purge the auto-exported space seed and repair the Region8 guard aggro. **Depends:** implementation-session authorization. **Decision:** D-CB02. **Advisor:** mission-systems-advisor; testing-validation-engineer for the guard.
...
**Status:** Done — merged to `main` in PR #646 (2026-09-17). **Scope title:** Chain-replay baseline for missions 640, 680 and 681-686. **Depends:** C01 (so the baseline is not asserting duplicates). **Advisor:** testing-validation-engineer.
...
**Status:** Done — merged to `main` in PR #646 (2026-09-17), as chain 1112, paired with C00. Effect 1634 confirmed a complete no-op today (`pulse_count=1` fails `is_pulsing()`, so `register_active_effect` returns `false` before any wire packet sends — pinned by `single_shot_scriptless_effect_registers_no_active_instance`); the item-19 cure path shares the same no-op shape. **Scope title:** Stasis Sickness applied on zone load, cure path verified. **Depends:** C01. **Decision:** D-CB04. **Advisor:** combat-systems-advisor; mission-systems-advisor.
...
**Status:** Done — merged to `main` in PR #646 (2026-09-17). Chosen approach: a per-entity deferred queue on `SpaceManager`, drained by the existing 100ms cell tick (same pattern as `pending_attack_tick`), rather than a spawned timer task — avoids new channel plumbing to reach `&mut SpaceManager` from outside the single-threaded cell message loop. **Scope title:** Honor `content_actions.delay_ms` in the executor. **Depends:** none. **Advisor:** testing-validation-engineer.
