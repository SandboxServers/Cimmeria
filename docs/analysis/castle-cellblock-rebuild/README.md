...
| D-CB08 | Unknown; worker checks in-client before implementing (took the recommended default). | C01 → C07 | C07's writer must do the UAT precheck described in its packet before writing any chains; if the client already shows a prompt, report back and close C07 as a non-goal instead of shipping a double prompt. |
| D-CB09 | Include the Castle-side Gerschon handoff now, as C09 (took the recommended default). | C01, C04 → C09 | No change to C09's scope as written. |
...
| M4 Boundary (C09) | Arrive in Castle near Gerschon, letter still active, 2573 or 5861 by archetype, 701 accepted. |
