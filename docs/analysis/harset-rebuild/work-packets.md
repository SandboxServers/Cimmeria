...
**Status:** Integrated 2026-09-18 into `content/harset-rebuild` (branch `harset/H01`, commit `00d4f49a`; [worknote](worknotes/H01.md)). Scope reduced the same day: part (2), the flag-2 region routing, moved to Castle CA10 (README U10 revised). Shipped: nullable `arrival_x/y/z/yaw` on `stargates` (seeded for no gate yet), `cell/arrival.rs` with `validate_gate_arrival` (navmesh check, respawner fallback, never the input; the placement call is one line in `gate_travel.rs` that CA10's rebase must carry into `on_stargate_passage`), and the DHD interaction emitting `onDisplayDHD` 120. 50 tests; the test review found and closed a CWD-relative navmesh-loading hole that had every gate test taking the unvalidated early return. Open question for M0: whether `rotation[2]` really carries yaw on the gate-travel message. **Scope title:** Safe gate arrival, stargate region flag, DHD interaction. **Depends:** none. **Decision:** D-H07, D-H08. **Advisor:** movement-teleport-advisor; server-authority-enforcer for the flag handler.
