...
## Update 2026-09-18 (~18:25 UTC): Castle CA00-CA10 all merged; PR #662 head 04526cfb

- Castle merged #660 (missions 702-704, `2a37ba4c`), the last implementable Castle PR. `origin/main` was merged into `content/harset-rebuild` a final time as `04526cfb` (one conflict: `chain_replay_tests/mod.rs` `mod` union, `castle_702_704_executor` before `entity_health_below`) and pushed. Coordinator-run at that commit on private DB `sgw_harset_pr662`: fmt clean, `cargo check --tests` clean, 206 content chains load, content-engine 192 + 8, services 2434 passed / 0 failed. The previous head `c8c562e8` had all 11 CI checks green; CI is re-running on `04526cfb`.
- **Merging #662 into `main` was deliberately NOT done**: it is a 146+ file PR and a hard-to-reverse outward action, so it waits for the owner's explicit go-ahead. When approved: confirm CI green, `gh pr view 662 --json mergeStateStatus` is `CLEAN` (if `DIRTY` again, merge `origin/main`, keep entity_templates `setval` = 248), check for any new Copilot review, then merge (prefer a merge commit so the fix SHAs quoted in the review replies stay reachable; a squash would orphan them).
- Then wave 2 as listed above (batches of 2-3; seed lanes first). Castle ledger items that touch Harset: H05 is unblocked (#652 merged), and `main` already carries CA10's gate events, so the operator guide warns against dialing to Harset on `main` until #662 lands.
