...
cluster is `is_stationary = true`. (Since NA29, 2026-09-25, five of them are mobile. See [NA29: five rows walk](#na29-five-rows-walk).)
...
On 2026-09-25 the owner agreed to un-stationary the six rows that the NA26 `harset.nav` moved onto the mesh. Each was checked against the shipped mesh for a full agent-radius disc (13 samples at r = 0.6, the METHOD.md ring) and for `find_path` to four points 3 m away and to the gate row:

| Spawn | `is_point_valid` (h, dy) | Disc r = 0.6 | Paths 3 m away | Path to the gate | Verdict |
|---|---|---|---|---|---|
| 303 `Harset_FormerRaJaffa` | 0.00, -0.31 | 13/13 | 4/4 `Ok` | `Ok` | **mobile** |
| 304 `Harset_FormerRaJaffa2` | 0.00, -0.09 | 13/13 | 4/4 `Ok` | `Ok` | **mobile** |
| 306 `SecondBug` | 0.50, -0.05 | 13/13 | 3/4 `Ok` (-Z is `NoEndPoly`, the mesh edge) | `Ok` | **mobile** |
| 307 `ThirdBug` | 0.40, -0.05 | 13/13 | 4/4 `Ok` (two stop about 3 m short of the goal, which is off-mesh) | `Ok` | **mobile** |
| 308 `Harset_ShieldTower1` | 0.00, **+3.44** | 13/13 | 4/4 `Ok`, but every end is 2-4 m off because of the height | `Ok` | **stays stationary** |
| 313 `Harset_PetbeQuarters` | 0.93, -0.22 | 13/13 | 4/4 `Ok` | `Ok` | **mobile** |

**Why 308 stays stationary.** Its seeded y (-41.36, the tower prefab's origin) is 3.44 m *above* the nearest walkable surface, and it passes `is_point_valid` only on the 4.0 m jump tolerance. It probably stands on one of the raised platforms the extractor does not decode. A mobile 308 would drop 3.4 m onto the mesh the first time it paths. PL-B-09 already rates this Y LOW. Re-pin it from a `.location` reading at the tower base, and then it can be made mobile too. `world57_mobile_placements_can_walk` fails once 308's dy drops below 2 m, to say so.

The six rows that were already on-mesh before NA26 (300, 301, 302, 305, 312, 314) and the three still off-mesh (309, 310, 311) were not part of the owner decision and stay stationary.
...
| PL-B-04 | 1326 Lan'toc Jaffa (accepts), spawn 303, template 204, tag `Harset_FormerRaJaffa` (**mobile since NA29**) | 57 | -160.00 | -41.28 | 84.00 | 4.6335 | MAP-LANDMARK (`JF-Tent03` row x -154.5, `GA-Barracks01` -198.1/84.4) + MAP-GEOMETRY + AUTHORED (chain 6335 expects the tag) | MEDIUM for the camp, LOW for the exact metre | **off-mesh** (nearest poly 6.72 m below, component 1364) - see the finding above; floor -41.3 confirmed (`obj_slab`, 120 m2, 241 tris at `y[-42, -41]`); heading faces the `HarsetRingLeft` pad (-194.5, 81.3), the way a player arrives in this quarter | Ring to the Jaffa Zone (left ring); two Jaffa stand in the camp street between the tent row and the barracks | `spawnlist.sql` spawn 303 |
| PL-B-05 | 1326 Lan'toc Jaffa (refuses), spawn 304, template 204, tag `Harset_FormerRaJaffa2` (**mobile since NA29**) | 57 | -160.00 | -41.28 | 89.00 | 4.4921 | same as PL-B-04 (chain 6336) | MEDIUM / LOW | **off-mesh**, same slab, 5.0 m from PL-B-04 so they read as two men | Beside PL-B-04 | `spawnlist.sql` spawn 304 |
...
| Test | Fails when |
|---|---|
| `world57_placement_rows_are_seeded_with_their_tags_and_templates` | any of the 15 rows is removed, renumbered, retagged, repointed at another template, has an `is_stationary` value other than the ledger's (mobile only for the NA29 five), or loses `respawn_secs` |
...
| `world57_mobile_placements_can_walk` | a mobile row (303, 304, 306, 307, 313) loses part of its r = 0.6 disc or its `Ok` path to the gate, **or** 308's height above the mesh drops below 2 m, which removes the reason it stayed stationary (NA29) |
