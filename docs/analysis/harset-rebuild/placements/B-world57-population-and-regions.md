...
| PL-B-07 | `SecondBug` basket, spawn 306, template 164 | 57 | -186.00 | -41.28 | 117.50 | 2.8993 | AUTHORED area (742 step 2504 text: "Hide the listening devices **in the Jaffa area** in Harset") + AUTHORED sibling (`FirstBug` spawn 224 at -176.7/125.3) + MAP-LANDMARK (`JF-Tent01` -185.4/114.9, `JF-Tent03` -184.5/121.8) | MEDIUM | **off-mesh** (2.14 m horizontally, 10.48 m below the nearest poly, component 1441) - the same relationship the authored `FirstBug` has; floor -41.3 confirmed (`obj_slab`, 286 m2, 462 tris); heading faces `JF-Tent01` | With 742 active, the north Jaffa camp holds three baskets a few metres apart | `spawnlist.sql` spawn 306 |
...
| PL-B-10 | Shield tower 2 console, spawn 309, template 243, tag `Harset_ShieldTower2` | 57 | -169.50 | -31.13 | 234.60 | 1.5126 | MAP-LANDMARK (`GA-TowMed00` at -166.1/234.8) | MEDIUM | **off-mesh** (2.04 m h, 9.06 m below component 1763); Y matches a real constructed floor - `obj_slab` gives 169 m2 / 338 tris at `y[-31.0, -30.5]` over exactly this box; heading faces the tower pivot | Ring to `HarsetRingLeftTop` (the palace terrace); the medium tower stands on it | `spawnlist.sql` spawn 309 |
