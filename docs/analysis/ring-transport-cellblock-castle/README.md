...
Server side, sequences 10187 (Teleport Out, event 8000) and 10188 (Teleport In, 8001) point at that path. **The client's `sequences` table is served from `data/cache/CookedDataKismetSeqEvent.pak`, not from the database**, so a seed row alone is invisible to clients. [`tools/pak_add_kismet_sequence.py`](../../../tools/pak_add_kismet_sequence.py) adds the matching PAK entries. Region 33 is not repointed yet; that is Phase 3.

> **Do not bump the PAK's `MetaData` version.** The first cut of this work did (7455 to 7456), on the assumption that clients would refetch. They do not. Category 1 has no per-key push, so the server answered `invalidate_all = true` with nothing pushed; the client dropped its whole sequence table, rewrote `Documents\My Games\Firesky\SGWGame\Cache.en-US\CookedDataKismetSeqEvent.pak` as an empty archive stamped 7456, and no Kismet sequence played at all, region 3's untouched ring included. The version is back at 7455 and a test in `crates/services/src/base/resources/tests/committed_paks.rs` pins it. New sequences reach a client the way the older custom ones (10015 and up) did: the identical PAK, at the same version, in the client's `SourceCache.en-us` and `Cache.en-US`. That makes the PAK part of the client patch, alongside the map chunks.

To test: close the client, run `map-backups\phase1-artifacts\Install-Phase1.ps1 rig`, copy the repo's `data/cache/CookedDataKismetSeqEvent.pak` over the client's `SourceCache.en-us` and `Cache.en-US` copies, stand at the Armory pad, and run `.net_seq 10187 3`. The rings should rise and flash as they do at region 3 with `.net_seq 1951 3`. `Install-Phase1.ps1 original` restores the stock chunk.
