...
> [!NOTE]
> NA38 settled the last two items. The real client orders its reliable stream itself, so the round-2 "combined conclusion" above does not hold for real players. See NA38.
...
- `Nub::processFilteredPacket` (`0x01580ad4`) splits packets on `FLAG_RELIABLE` (`0x10`). Reliable packets go to `UnAckedHandler::queueAckForPacket` (`0x0158cba0`). It queues the ACK first. At `seq == inSeqAt` (`ChannelInternal+0x50`) it advances and chains on every packet already buffered behind it, and the caller processes that chain in order. Ahead within the window, it buffers the packet in the slot table at `+0x40` (`"Buffering packet #%d above #%d"`, `0x01b1a040`). Behind, or already buffered, it drops the packet. Too far ahead, it drops the packet and warns.
- Unreliable packets skip the window. `FUN_0158bb50` dedups them against `+0x128`, and they are processed on arrival.
- `inSeqAt` starts at `0x10000000` (`ChannelInternal` ctor `0x0158c7b0`) and adopts the first reliable sequence. The window is 512: the `Channel` ctor (`0x01576bf0`) writes `0x200` to `Channel+0x2c`, which becomes `ChannelInternal+0x30`.
- Messages for an entity the client has not created yet are not lost. `EntityManager::onEntityMoveWithError` (`0x00dd1650`, vtable slot `0x019ce99c`) stores the latest position in a pending-entity map at `EntityManager+0x30`. The create handler (`0x00dd2270`, slot `0x019ce98c`) consumes that record when it builds the entity. `onEntityMethod` (`0x00dd2b80`) and `onEntityProperty` (`0x00dd29d0`) copy an unknown entity's message into a per-id buffer at `+0x3c`. The replay of that buffer at create time was not traced.
- Server emit policy (unchanged): `CREATE_ENTITY`+`UPDATE_AVATAR` and the cascade go reliable, in that order, from one `entered_aoi` call. Only the per-tick `entity_moved` position relay and `tickSync` go unreliable.

**Verdict on the owner's one-way visibility report:** NA37 round 2's mechanism does not reach real players. When a peer's `CREATE_ENTITY` is lost, the real client holds the cascade until the retransmit fills the gap, and it caches the early position relays. The entity appears one retransmit later, with its appearance, at its latest position. The round-2 repro failed only because the harness handed bundles over in raw arrival order, which the real client never does. The report stays unexplained. The next candidate is server logic order: a reliable message about B reaching A from a task other than the one that sent A's create for B. The harness can now catch that, because it orders the way the client does. Player-to-player visibility stays `NT`.

**Fix:**

- **Mercury.** `Channel::receive_parsed` (`crates/mercury/src/channel/rx_order.rs`) is the client's gate on a `ParsedPacket`. It releases reliable packets in sequence order, holds everything behind a gap, drops duplicates, and passes unreliable packets straight through. Released fragments go to the assembler. It returns an `RxDelivery` with the bundles, the ACK owed and an `RxOutcome`. A packet beyond the window is not acked, so the sender keeps resending it. `anchor_rx_seq` pins the starting sequence; without it the channel adopts the first sequence, as the client does. `RX_WINDOW_SIZE` goes from 64 to the client's 512. `receive_packet` now shares the same window, and its sequence arithmetic masks to 28 bits.
- **Harness.** `LoopbackPeer`'s recv pump goes through `receive_parsed`, and its peers anchor at 0. `GameSession` anchors at `handshake::FIRST_CHANNEL_SEQ` (3), the first sequence after the phase-3 reply (1) and time-sync (2).
- **Server.** `handle_encrypted_datagram` runs every client packet through the session channel's gate before dispatch. The session channel (`login::new_client_channel`) adopts the client's first reliable sequence, as the client does for ours. The one capture shows the client starting at seq 0 (`castle_cellblock_head`, flags `0x58`), but pinning 0 would hang any client that started elsewhere: every packet would wait behind a gap that never fills. Before this, a client retransmit whose original had arrived (our ACK was lost) was dispatched twice, and a reliable client message behind a gap ran ahead of it. No server handler depended on raw arrival order. Unreliable client packets still go straight through. Reliable ones, including movement (`0x03`) when the client sends it reliably, now run in the client's send order, as on a BigWorld BaseApp.
- **Stall watchdog.** `Channel::check_rx_stall` runs on each 100 ms tick, from the server's retransmit tick and the harness `tick`. When one reliable gap has blocked delivery for more than 2 s (`RX_STALL_WARN_MS`), it logs a WARN, repeated at most every 10 s (`RX_STALL_REWARN_MS`) for that gap. The server counts each gap once in `mercury_rx_stalls_total`. The watchdog never skips the gap. The client doesn't skip one either: `queueAckForPacket` only advances on the exact next sequence.
- **Telemetry.** `mercury.rx_order` records `event=buffered` and `event=duplicate` at DEBUG. At WARN it records `event=out_of_window` (`reason=beyond_rx_window`) and `event=rx_stall` (`reason=reliable_gap_unfilled`, with `peer`, `expected`, `first_buffered`, `last_buffered`, `buffered`, `depth` and `stalled_ms`). The target is added to `OTEL_FILTER`.

**Acceptance.** Each guard below fails when the gate is bypassed (every reliable packet delivered on arrival):

- `channel::tests::rx_order` (9 unit tests): a gap holds and releases in order; duplicates below the window and in a buffered slot are dropped but acked; unreliable packets pass a gap; beyond-window packets are unacked; the unanchored channel adopts the first sequence; the anchored channel recovers a lost first packet; the 28-bit wrap; a reversed reliable fragmented bundle; `RX_WINDOW_SIZE == 0x200`. Six of them fail under the bypass.
- `lomiada_single_packet_gap_recovers_via_retransmit` now asserts that only packet-0 and packet-1 are delivered before the retransmit, then packet-2 to packet-9 in order, with no duplicates.
- `connect_loop::encrypted::rx_order_tests`: `a_client_packet_behind_a_gap_waits_for_the_retransmit` and `a_retransmitted_duplicate_is_acked_but_not_dispatched_twice` drive real encrypted datagrams into `handle_encrypted_datagram` and watch the cell channel. `a_client_whose_reliable_stream_starts_at_1234_is_dispatched_normally` uses the channel `handle_login` builds. It fails if that channel is pinned at 0, because seq 1234 falls beyond the window.
- `channel::tests::rx_stall`: a stream starting at 1234 delivers normally. A gap left open past 2 s is reported once, with its expected and buffered sequences and its depth, then throttled, and then re-warned without being counted again. A filled gap is never reported. A new gap behind a filled one restarts the clock. The two positive watchdog tests fail with the watchdog disabled.
- `burst_drop_of_peer_create_entity_recovers_via_retransmit` is un-ignored and passes on an unchanged server. It requires that only unreliable position relays for B arrive before B's retransmitted create. It needs at least one such relay, which proves the drop landed on the create. It then requires B's `BEING_APPEARANCE`. Under the bypass it fails with 16 reliable cascade messages ahead of the create.
- Runs: nextest on mercury, services, wireclient and server all pass. The live-DB services lib (3308) and wireclient (42, including all five two-client tests) pass against the worktree database.

**Outstanding:** the client's unreliable dedup (`+0x128`) is not modelled, so a network-duplicated unreliable datagram is still delivered twice. Tracing the `+0x3c` replay at create would confirm that an early method or property is applied rather than discarded. The owner's next two-client session still needs capturing.
