...
- **Evidence.** SGW.exe resolves no hits; its only cover stat string is the `CoverQRModifier` Lua label. The units come from the client's `alias.xml` stat comments: `coverDefense` -0.01 QR per point (line 235), `coverAccuracy` +0.01 against a target in cover (234), `coverQRModifier` 1 QR per point for whichever side is behind cover (216). The cooked text agrees that 100 points = 1 QR (1729 vs 4299, 1450 vs 4995). "Cover Penetration" is `coverAccuracy`. Magnitude: the effect row's +100 (4565, like 1746, 1747 and 2003), not the ability text's +200. See [combat-system.md](../../gameplay/combat-system.md#cover-in-the-qr-roll-na32).
- **Formula.** `combat::cover_shift` (`combat/damage/cover_qr.rs`): a defender in cover loses `max(0, coverDefense x 0.01 + coverQRModifier - attacker coverAccuracy x 0.01)` of QR; an attacker in cover gains its `coverQRModifier`. `SpaceManager::cover_standing` (`space_manager/cover_hit.rs`) decides "in cover": an NPC within 1.5 u of its held slot, a player within 1.5 u / 2 u of the nearest node, and the attacker not flanking (`is_flanked`, 20 degrees past side-on). `abilities/damage_apply/cover_roll.rs` adds it to the QR and writes `abilities.qr event=cover_resolved` (`defender_cover`, `flanked`, `cover_defense_applied`, ...). No wire field changes layout.
...
- **Tests (each revert-proven).** `cover_qr` units; `cover_hit` standing (slot, walk-to, flank band, player floor); `damage_apply/cover_tests.rs` through `apply_damage_to_target` (covered NPC takes exactly the -1.0 QR hit, flanked and away-from-cover take the exposed hit, covered player, the telemetry row); `qr` distribution tests (`expected_damage_rises_with_qr` and the band tests fail on the python order, as does the covered-NPC test); `tests/npc_ai/step_back.rs` (steps back, fires outside 2 u, melee, stationary, cooldown, in flight, dead-zone hold, in cover, flanked) and two on the real `castle_cellblock.nav` (the step lands on the mess-hall floor; cornered against a wall it fires). Two `ability_range` tests moved their player from 1 u to 4 u, outside the comfort range.

**Acceptance:** UAT: shoot a Cellblock guard in its cover slot from in front, then from its flank; SigNoz `abilities.qr event=cover_resolved` shows `defender_cover=in_cover cover_defense_applied=1.0` then `flanked=true cover_defense_applied=0.0`. Walk into a guard's face: it backs off about 3 u (`decision_outcome=step_back`) and no more than once per 3 s.
