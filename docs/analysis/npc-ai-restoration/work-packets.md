...
**Status:** Done (2026-09-25). **Scope title:** Reproduce shared-world player visibility end-to-end with two real wire clients, following up on NA34's in-process investigation. **Advisor:** aoi-witness-broadcast.

**Context:** NA34 drove both `EnteredAoI` arrival orders at the base-dispatch level (in-process `ConnectedClientState` sessions, no sockets) and could not reproduce the owner's report of one-way player visibility in Castle/Harset. This packet goes one level further: real wire bytes, through the real SOAP auth → Mercury handshake → world-entry sequence, against a real spawned server, since the owner's report could plausibly be a wire encode/decode bug the in-process test can't see.

**Scope:**

- **`cimmeria-wireclient` grew a real UDP socket loop** (`src/session.rs`'s `GameSession`), closing wireclient's Phase 1.5 gap for this scenario by reusing `cimmeria_mercury::test_harness::LoopbackPeer` — the Tier 2 loopback harness's Channel driver — against a real `BaseService` socket instead of a paired test peer. No second reliable-delivery/reassembly implementation was written.
- **A structural server→client bundle decoder** (`src/bundle.rs`) ported from `tools/pcap_dissect.py`'s `SERVER_MSG_FORMAT` table, extracting msg_id/entity_id/class_id/method_index without full per-argument semantics (that remains wireclient Phase 3).
- **`crates/wireclient/tests/two_client_castle_visibility.rs`** (live-DB): two real `GameSession`s enter a real spawned `Orchestrator`'s Castle world (world 8) via sentinel `sgw_player` rows this test inserts and cleans up (one seeded GM account, one sentinel non-GM account this test also inserts/removes). Asserts, in **both** arrival orders: each witness's decoded wire bytes carry the other's `CREATE_ENTITY`, a `BEING_APPEARANCE` (method 26) cascade entry, a movement relay (`UPDATE_AVATAR` family) after a client sends `AVATAR_UPDATE_EXPLICIT`, and a `leaveAoI`/`entityInvisible` after a `DISCONNECT`. A companion test asserts a character placed >100m away (`CellEntity::aoi_radius` default) is **not** introduced within a multi-second window — the negative control against a harness bug that made the positive assertions pass trivially.
- **A real test-infrastructure bug found and fixed along the way, initially indistinguishable from the reported one.** The first runs against a real spawned `Orchestrator` showed **zero** cross-player introduction in either direction — exactly the owner's symptom. Root cause: `CellService::start()` loads `entities/spaces.xml` from the process's *current working directory*-relative path `"entities"` (`crates/services/src/cell/service/mod.rs`, no `ServerConfig` override exists), and `cargo test`'s CWD for an integration test binary is the package directory (`crates/wireclient/`), not the repo root — so `CreateEntity` logged `"Unknown world: Castle"` and the two player entities were never registered in any real `SpaceManager` space, so they could never appear in each other's AoI regardless of anything downstream. Fixed by `chdir`-ing the test process to the repo root before starting the server (`chdir_to_repo_root_for_entities_xml` in the test file). This is a trap for *any* future test that spins up a real `Orchestrator`/`CellService` from an integration-test binary outside `cimmeria-services` itself, not something specific to AoI — worth remembering.
- **Confirmed, not a bug: every player-ghost introduction is class-flattened to `SGWPlayer` (0x02).** The first honest test run (after the CWD fix) failed a class-byte assertion: a GM's ghost showed up on the witness's wire as `SGWPlayer`, not `SGWGmPlayer`. This is `player-ghost-aoi-cascade.md`'s documented, intentional "GMs are introduced as plain players" gap (`connect_entity` stamps `class_id = 0x02` for every player's cell identity) — not a new finding. The test was corrected to assert the documented behavior instead of flagging it.
- **Result once both test bugs were fixed: full pass, both directions, both arrival orders, GM and non-GM.** `AoI bundle: flushed 9 messages` fired for both witnesses in server logs at the moment each new player's `onClientReady` completed. This is a **positive** result at the deepest layer tested yet — real wire bytes, real socket, real spawned server — agreeing with NA34's in-process finding.
- **Root cause of the owner's report: still not found.** Two investigations at two different layers (NA34's in-process dispatch, and NA37's real wire path) have now both failed to reproduce a permanent one-directional failure; NA37 additionally rules out a wire encode/decode bug specifically, which NA34 could not. Neither test proves the literal SGW.exe client renders correctly — `wireclient` is a protocol-conformant test client, not the game client, and its bundle decoder does not check per-argument semantics deeply enough to catch every possible client-side misparse.
- **Most likely remaining explanation: client-side, or environment-specific.** Candidates identified but not verified against the binary: (a) the client's own AoI/ghost-slot management losing an entry under some timing condition neither server-side test reproduces (no server bug, client renders nothing for a ghost it never asked to re-sync); (b) a race specific to the owner's actual session (packet loss/reorder/latency, or a timing window) that neither the in-process nor the wireclient test replicates since both run over lossless localhost with no chaos policy; (c) `aoi.player_ghost_incomplete`/`aoi.entered_no_witness_addr` telemetry (added by NA34) not yet observed on a real report window — the owner's next session should be captured live and checked against both that telemetry and this test's assumptions.

**Acceptance:** `cimmeria-wireclient` unit tests pass (`bundle.rs`'s 6 decoder tests, all 33 lib tests); `two_client_castle_visibility.rs`'s two tests pass locally against a live DB, stable across repeated runs (not yet wired into CI — see `docs/architecture/wireclient.md` Phase 7). `cargo fmt --check` and `cargo +1.98.1 clippy -p cimmeria-wireclient --all-targets -- -D warnings` both clean.

**Outstanding:** wire the live-DB wireclient tests into a `wireclient-e2e` nextest profile / CI job (wireclient.md Phase 7); capture a real SigNoz `aoi.introduce` trace from the owner's *next* live session and cross-check against `aoi.player_ghost_incomplete` / `aoi.entered_no_witness_addr`; if both come back clean again, the fault is very likely client-side and the next step moves to the game-archaeology-specialist / bigworld-engine-advisor agents to inspect the client's own ghost-slot lifecycle in Ghidra. Consider a `ServerConfig`/`CellService` setter for `entities_dir` so future tests that spin up a real cell service don't have to `chdir` the process.
