...
- **Server.** `handle_encrypted_datagram` runs every client packet through the session channel's gate before dispatch. `handle_login` anchors the channel at 0: the client's first reliable packet is seq 0 (`authenticate`+`enableEntities`, flags `0x58`, in the `castle_cellblock_head` capture). Before this, a client retransmit whose original had arrived (our ACK was lost) was dispatched twice, and a reliable client message behind a gap ran ahead of it. No server handler depended on raw arrival order. Unreliable client packets still go straight through. Reliable ones, including movement (`0x03`) when the client sends it reliably, now run in the client's send order, as on a BigWorld BaseApp.
- **Telemetry.** `mercury.rx_order` records `event=buffered` and `event=duplicate` at DEBUG, and `event=out_of_window` at WARN with `reason=beyond_rx_window`. It is added to `OTEL_FILTER`.
...
- `connect_loop::encrypted::rx_order_tests`: `a_client_packet_behind_a_gap_waits_for_the_retransmit` and `a_retransmitted_duplicate_is_acked_but_not_dispatched_twice` drive real encrypted datagrams into `handle_encrypted_datagram` and watch the cell channel.
