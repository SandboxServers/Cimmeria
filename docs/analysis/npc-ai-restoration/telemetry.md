...
| `movement.npc` `event=stale_velocity` | WARN, 10 s per NPC | Velocity non-zero while the position has not changed for 3 movement ticks (300 ms) | `vx, vy, vz, speed, still_ticks, path_state` (`empty`, `stalled`), `nav_path_len, ai_state, movement_type` (the dedup cache, for NA10's before/after). This is the running-in-place detector (S1). |
...
| `npc_ai` `event=stuck` | WARN, 15 s per NPC | Fighting and chasing (a path exists and the target is out of range) while `npc_to_target` has not shrunk for 3 AI ticks | `npc_to_target_history, nav_path_len, los, next_wp, decision_outcome`. **As built:** "chasing" is the fight's out-of-range branch (`chase, hold_no_repath, repath_degenerate, no_path`) with a non-empty path; "shrunk" means by at least 0.5 |
...
| Target / event | Level | When | Fields |
|---|---|---|---|
| `npc_ai` `decision_outcome=no_cover` | DEBUG | Replaces the silent `NoCover => {}` arm | `reason` (`use_cover_false, stationary, no_candidate_in_radius, reserve_lost, in_range_no_better_slot`, plus the defensive `index_miss`), `candidates_scanned, reserved_skipped, search_radius, cover_nodes_loaded` |
...
| `cover.coverage` `event=space_summary` | INFO at space load; WARN when unusable | Per space: nodes inside the space bounds and inside navmesh coverage | `nodes_total, nodes_in_bounds, nodes_on_mesh, sets_in_bounds, sets_origin_centred`, WARN `reason`. **As built (NA02):** "WARN when 0 on mesh" does **not** fire on Cellblock with today's seed — the seed's Y column is a horizontal UE3 axis and ~3,000 of the ~7,000 in-bounds nodes land on the mesh's `y ≈ 0.2` ground plane by coincidence. The row also WARNs with `reason = prefab_local_coordinates` when more than half the sets in bounds straddle the world origin (centroid within the set's own extent of `(0, 0, 0)`), which is the C1 signature and does fire |
