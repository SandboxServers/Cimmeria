...
| Target / event | Level | When | Fields |
|---|---|---|---|
| `npc_ai` `decision_outcome=no_cover` | DEBUG | Replaces the silent `NoCover => {}` arm | `reason` (`use_cover_false, stationary, no_candidate_in_radius, reserve_lost, in_range_no_better_slot`, plus `no_world` (the space has no `resources.worlds` id, so no cover index) and the defensive `index_miss`), `candidates_scanned, reserved_skipped, search_radius, cover_nodes_loaded` |
