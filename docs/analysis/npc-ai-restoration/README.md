...
| D-NA14 | APPROVED (owner, 2026-09-25: "work on all still open items") | **A player's targeted ability needs line of sight at fire time, and every eye is at its body set's height.** Where the world ships an occluder, a player ability aimed at another entity is refused with `onErrorCode(0, ability, 39)` (`CONDITION_FEEDBACK_LOS`) when every ray is blocked. The rays are eye to eye; to the target one tick back along its velocity; from the player one tick ahead; and to the target's eye 0.35 m to each side. No occluder, or an eye off its grid, never refuses. NPC launches are not re-checked, because the fight tick checked them in the same tick. Auto-cycle sends the error once, then waits armed and silent. Eye heights come from `resources.body_sets.eye_height`, the top of the reference mesh's bounds less 0.12 m, with 1.5 m where a body set has no mesh. They apply to NPC aggro, assist, attack and cover sight too. | NA31 ([finding](../../reverse-engineering/findings/being-eye-heights.md)). `ErrorStrings.pak` `_39` is the only line-of-sight entry with text ("You do not have Line of Sight to your target"); `_40` is its moniker. The script pawn (`SGWGamePawn`) inherits stock UE3 `BaseEyeHeight` 64 and `CollisionHeight` 78 for every being, but the reference meshes differ: a human male is 1.93 m tall, a Jaffa male 2.24 m and a rat 0.27 m. `BS_JaffaMale`, the body set of 50 templates, had no `body_sets` row and gains one. **Known cost:** Jaffa and human eyes are now 0.3-0.6 m higher than NA27's 1.5 m test height, so standing NPCs see over High cover (1.524 m), and players see them over it. |
