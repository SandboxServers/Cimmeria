...
| D-NA15 | APPROVED (owner, 2026-09-25: "work on all still open items") | **Cover counts in the hit roll, and ranged NPCs step back.** (1) A defender at a cover node that faces its attacker takes `max(0, coverDefense x 0.01 + coverQRModifier - attacker coverAccuracy x 0.01)` off the QR; an attacker behind cover adds its own `coverQRModifier`. "Faces" is the AI's own flank test (not more than 20 degrees past side-on), so a flanked defender gets nothing. An NPC counts at its held slot (within 1.5 u), a player at the nearest node on its floor. Cover Stance is the effect row's +100, so -1.0 QR; the ability text's +200 is not used. To make any QR shift mean what the designers wrote, the beta roll's branches are swapped: the mean now rises with QR. (2) A mobile ranged NPC (its ability has a `min_range`, or all its abilities are ranged) attacking in place, not in cover, steps back when its target is inside `max(min_range, 2 u)`: straight away, slid along the navmesh, to that range plus 3 u, at most once every 3 s. Melee, stationary and in-cover NPCs never do. | NA32 ([work-packets.md](work-packets.md#na32)). The units are the client's own `alias.xml` stat comments (`coverDefense` -0.01 QR per point at line 235, `coverAccuracy` +0.01 at 234, `coverQRModifier` 1 per point at 216), and the cooked text states 100 points = 1 QR twice (ability 1729 vs effect 4299, 1450 vs 4995). "Cover Penetration" is `coverAccuracy` (1450 -> 1741). The effect rows 1746, 1747, 2003 and 4565 all say +100; ability tooltips contradict their own effects elsewhere (1452 -> 1743). SGW.exe resolves no hits (only the `CoverQRModifier` Lua label exists). Python applied QR the wrong way round: at QR +1.5, 45% of rolls were Miss/Glancing and expected damage was flat in QR, so cover would have changed nothing but the floaters. Every seeded NPC ability has `min_range = 0`, so the old min-range backup never ran. **Known cost:** with the fan-authored `(1 + qr)` damage term, a guard in cover takes about 5% of the damage an exposed one does. The lever is a `CoverDefense` NVP on effect 4565. |
