...
| M4 | Surrender or panel diagnosis reveals the crystal; Bravo or Muelbach (bunker above Bravo) grants 2790 once; Human reports to Marsh, Jaffa to Moh'katan, never both; DHD Livewire; gate opens after 4 s; crossing plays and lands on Harset once. **Harset dependency:** the landing half of this check cannot pass until Harset's M0 placement session pins gate 3's arrival (the nullable `arrival_*` columns exist but no gate is seeded and world 57 has no respawner row). After Harset PR #662 the crossing is refused by design: the player stays in Castle and one `arrival_unrecoverable_off_mesh` warn is logged per attempt. On a refused crossing 6113 and `stargate_crossed` still fire first (the contract forbids a second `validate_gate_arrival`). |
