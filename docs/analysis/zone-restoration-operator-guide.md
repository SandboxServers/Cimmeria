...
| Placements (this PR) | Gate-3 arrival pin at (-5.0, -68.99, 33.0) (dropped by NPC-AI NA29 on 2026-09-25: travellers now arrive on the gate row); respawners for worlds 57, 69 and 70; return door 6007 open; 15 world-57 spawns and 8 named regions; 10 Command Center spawns and 3 interior regions; 15 encounter-anchor candidates (ledger only, no chains) |
...
| # | Where / command | Do | Expect | If it fails, report |
|---|---|---|---|---|
| C4.0 | **Read before C4.1-C4.4** | Finish mission 708's DHD minigame (check B19) first | Every row below assumes you hold Harset's address. Before Harset H55 nothing in the game granted it: a created character's address book is empty, the dial UI only lists known addresses, and after Harset H06 the server refuses a dial to an address you do not hold, with a feedback message and no gate. A GM can still top their own book up with `gmDHD`, which does not persist | If you cannot select Harset in the DHD at all, the grant did not land — that is B19, not a gate-arrival failure |
...
| C4.3 | After the placement PR (`harset/placement`) merges, and **before** NA29 | Dial and cross again | You land on the plaza in front of the gate at the pinned arrival point (PL-A-01), facing into the plaza, and can move | Where you landed and which way you faced |
| C4.4 | After NPC-AI NA29 (the pin dropped, the NA26 `harset.nav`) | Dial and cross again | You land on the gate dais at the gate row (-0.08, -67.27, 38.01), facing down the plaza, standing on the floor. You can walk off straight away, and you are **not** sent back through the gate, even if another player has it open | Where you landed, which way you faced, whether you could move, and whether you travelled again without dialling |
...
| # | Where / command | Do | Expect | If it fails, report |
|---|---|---|---|---|
| C5.1 | Castle gate, to Harset (PL-A-01, dropped by NA29) | Dial and cross (C4) | Before NA29: you land on the plaza at about (-5.0, -68.99, 33.0). After NA29: you land on the gate dais at about (-0.08, -67.27, 38.01). Either way you face down the plaza (away from the gate) and stand on the floor | Where you actually landed, which way you faced, whether you could move |
