...
| D15 | G05 (`.goto`/`.summon`/`.gotolocation`) design round, approved 2026-09-17: `.gotolocation` targeting a world with multiple loaded instances joins the first/default loaded instance (matches legacy's own lack of disambiguation and the existing `resolve_space_id_fallback` pattern) — no new instance-selector argument. The entity being moved (target-or-caller) is restricted to **players only** for any cross-space/cross-world transfer in G05's three commands — an NPC target is rejected with a clear error, matching P26's precedent, since cross-world transfer requires the player-client world-entry handshake (`RESET_ENTITIES` → `pending_world_entry` → re-enter) that NPCs don't have. Name resolution itself already follows D05 (online, same-service, exact match, join the actual instance) and needs no further disambiguation policy — legacy's own `PlayersByName` lookup is a flat exact-match dict with no fuzzy/ambiguous-match handling. |
