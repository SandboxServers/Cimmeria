...
**Update (2026-09-17, P18 integration):** a fourth call site with the same class of bug was found: `console/entity.rs`'s `.lookat` handler (`look_at`, lines ~226-250) writes a Cartesian unit vector into `direction` instead of a bare yaw into `.y` — wrong under the now-fully-confirmed `[pitch, yaw, roll]`-radians semantics (see `.claude/agent-memory/rust-gameserver-dev/cell-entity-direction-semantics.md` for the complete evidence chain, now settled beyond the P02/P08 "candidate" framing). Folded into this packet's scope per the P18 worker's recommendation — fix `.lookat` alongside `.facing`/`.combatinfo`/native `.rotation` in the same pass, since all four share one root cause and one fix shape (write/read `.y` directly, no vector math). See also [P49](work-packets.md#p49) for a related but distinct, higher-severity bug in the wire-decode path (not a console command) that P18 also surfaced.

### P49

**Status:** Implemented, UATPending (2026-09-19, `fix/playtest-npc-facing-follow` — `update_entity_position` now stores client facing as radians in `[pitch, yaw, roll]` slots and `pack_angle` wraps instead of saturating; see `docs/analysis/playtests/2026-09-18-colo-castle/README.md` section 10). Previously: Ready (opened 2026-09-17, P18 integration — found while confirming `CellEntity.direction`'s semantics, not a console-command packet). **Commands:** none — this is a client-input wire-decode bug affecting every moving player's broadcast facing, not a dot/native command. **Depends:** none (self-contained fix). **Advisor:** movement-teleport-advisor, network-security-auth (wire-format correctness).
**Entries:** [avatarUpdateExplicit decode](../../../crates/services/src/base/connect_loop/encrypted/mod.rs) (~line 278), [`update_entity_position`](../../../crates/services/src/cell/space_manager/entities.rs) (the `direction: [i8; 3]` parameter every server-authoritative mover — `.gotoxyz`, native `gmGotoXYZ`/`gmGoto`/`gmSummon`, respawn, ring transport, the content executor's transport action, and now `.location` — passes through unconditionally).
**Scope:** `avatarUpdateExplicit`'s handler reads three raw packed-angle bytes off the wire (`payload[32..35] as i8`) and passes them straight through as `BaseToCellMsg::EntityMove.direction: [i8; 3]`, which `update_entity_position` writes into `cell_entity.direction` via a bare `as f32` cast — **no unpacking** (`pack_angle`'s inverse is `byte * (2π/256)`, never applied on the inbound path). Meanwhile `pack_angle` on every *outbound* AoI packet assumes `direction` already holds radians. Net effect: a moving player's own client-reported facing is stored as a tiny integer-range value instead of radians, then mis-scaled by ~40x when re-broadcast to witnesses via `mercury/aoi/update.rs`'s `pack_angle(direction[1])` — **every witness sees a wrong, likely wildly incorrect, facing for every moving player**, continuously, not just after a GM command. This is `update_entity_position`'s `[i8; 3]` parameter shape itself being the wrong contract for anything except this one inbound wire path — P18 found that every *other* caller of `update_entity_position` has to defensively work around it (pass `[0, 0, 0]` and separately restore `direction`, per P18's own fix) precisely because the parameter's real meaning ("raw wire bytes, needs unpacking") doesn't match what any server-authoritative caller actually has (already-correct radians). Consider changing the signature to `Option<[f32; 3]>` (radians, `None` = preserve existing facing) with the unpacking math moved into the one legitimate wire-decode call site, removing the footgun for every future caller rather than requiring each one to remember the P18 workaround.
**Acceptance:** a wire-format/round-trip test proving a client-reported facing byte triple decodes to the correct radians value and reaches witnesses' outbound packets unchanged (byte-for-byte round-trip through decode → `direction` → `pack_angle` re-encode); confirm no existing caller of `update_entity_position` regresses if the signature changes (P18's `.location`, `.gotoxyz`, native travel, respawn, ring transport, content-executor transport — grep for all call sites before changing the signature). **Exclude:** any change to `pack_angle`/`unpack_angle` themselves if they're already correct in isolation — the bug is specifically the missing unpack step on this one inbound path, not the packing math.
...
**Status:** Integrated (2026-09-17, `legacy-command-parity`). `.lookat` turned out to already be implemented (P13/P14-era `console/entity.rs`), so this packet was `.location`/`.rotation` only, in a new `console/placement.rs`. **The Euler-orientation stop-condition did not trigger** — resolved with direct evidence, not a guess: `CellEntity.direction` is `[pitch, yaw, roll]` in radians (confirmed via `pack_angle`'s `2π/256` divisor plus five legacy sites, most decisively `SGWPlayer.save` persisting `heading = rot.y`), so legacy `target.rotation = Vector3(x, y, z)` maps 1:1 onto `direction.x/.y/.z` — no vector math, no design sign-off needed. Findings and evidence chain: `.claude/agent-memory/rust-gameserver-dev/cell-entity-direction-semantics.md`. **Real bug found and fixed, not just a new command shipped:** `SpaceManager::update_entity_position`'s `direction: [i8; 3]` parameter is written unconditionally, and *every* server-authoritative mover (`.gotoxyz`, native `gmGotoXYZ`/`gmGoto`/`gmSummon`, respawn, ring transport, the content executor's transport action) passes `[0, 0, 0]` — silently zeroing the moved entity's facing. `.location` captures `direction` before the grid write and restores it after; copying `.gotoxyz`'s call verbatim would have shipped the same bug under a new name. Legacy's own bug (a 1- or 2-arg invocation silently no-ops the write then reports as if nothing was asked) corrected per D02: only 0 or 3 args accepted, anything else rejected with no mutation and no readout. `.rotation` on a player target is server-side + witness-visible only — `compose_forced_position_body` carries no angle field, so there's no wire mechanism to snap a player's own camera yaw; matches legacy exactly (posing an NPC, the realistic use, works fully). Opened [P49](work-packets.md#p49) for a related, higher-severity, out-of-scope-here bug this packet's investigation surfaced (the wire-decode path for player-reported facing skips unpacking entirely); folded `.lookat` into [P48](work-packets.md#p48)'s scope (same root cause). Worknote/handoff: [worknotes/p18.md](worknotes/p18.md), [handoffs/p18.md](handoffs/p18.md). **Commands:** `.location`, `.rotation`, `.lookat`. **Depends:** P26. **Advisor:** movement-teleport-advisor, aoi-witness-broadcast.
**Entries:** [console placement](../../../crates/services/src/cell/console/placement.rs), [native travel](../../../crates/services/src/cell/cell_methods/gm/travel.rs), [teleport](../../../crates/services/src/base/world_entry/teleport.rs).
...
**Status:** Closed out (2026-09-17) — all three children (P44, P45, P46) Integrated. **Commands:** `.goto`, `.summon`, `.gotolocation`. **Depends:** P26. **Advisors:** movement-teleport-advisor, aoi-witness-broadcast, database-persistence.
...
**Status:** Integrated (2026-09-17, `legacy-command-parity`) — G05 closed out. Thin adapters over P44 (`find_online_player_by_name`) and P45 (`transfer_player_to_space`): `move_subject` picks between P26's same-space snap and P45's full cross-space transfer per the *resolved destination*, not per command, so a same-space `.goto`/`.summon`/`.gotolocation` pays no loading-screen cost. **One design call made, grounded directly in D15's own wording rather than assumed:** D15 restricts the NPC ban to "any cross-space/cross-world **leg**" and explicitly cites continuity with P26's same-space precedent — so a genuinely same-space move routes around P45's unconditional `is_player` check (which still fully owns and enforces the cross-space case). `TransferOutcome` is matched exhaustively (`SameSpace` performs no position move; an `is_ok()` adapter would have silently reported a teleport that never happened). Legacy's four GM strings (`Player.py:298-365`) reproduced verbatim; `Ambiguous` (P44's outcome, no legacy equivalent) gets new wording. `console/travel.rs` split into `travel/mod.rs` (shared mechanisms + `.gotoxyz`) + `travel/named_destination.rs` (this packet's three commands) at the 508-line soft-cap boundary — `.gotoxyz`'s own behavior and all 10 of P26's tests unchanged. **Known gaps, inherited not introduced:** P44's `InTransition` reachability gap (mid-gate-travel reports "not available" where legacy said "not reachable"); the cross-world success feedback is queued behind `GateTravel` on the same channel, so a GM moving themselves may still be mid-loading-screen when it lands — sending it earlier would claim an unaccepted transfer, so left as documented UAT behavior. Worknote/handoff: [worknotes/p46.md](worknotes/p46.md), [handoffs/p46.md](handoffs/p46.md). **Commands:** `.goto`, `.summon`, `.gotolocation`. **Depends:** P44, P45. **Advisor:** movement-teleport-advisor, aoi-witness-broadcast.
**Entries:** [console travel](../../../crates/services/src/cell/console/travel/named_destination.rs), [native travel](../../../crates/services/src/cell/cell_methods/gm/travel.rs) (reference only — native travel is same-service numeric/same-space, not proof of these three commands' named/cross-instance contract).
**Scope:** the three thin command adapters over P44+P45: `.goto <name>` moves caller-or-selected entity to the named player's position/instance; `.summon <name>` moves the named player to the caller-or-selected entity's position/instance (as shipped; changed 2026-09-20 to always the **caller's** — see [worknotes/p46.md](worknotes/p46.md) §6); `.gotolocation <worldName> <x> <y> <z>` moves caller-or-selected entity to explicit coordinates in the named world (D15's first/default-instance rule), rejecting an unknown world with legacy's exact wording (`deprecated/python/cell/commands/Player.py:344-365`, confirmed: `"Unable to find world: %s"`). Legacy's own `'Player is not available on this CellApp'` / `'Player is not on any reachable space'` / `'Teleporting to player <%s>'` / `'Summoning player <%s>'` wording (Player.py:298-341) should be matched or deliberately deviated-from with a note, not silently reworded.
...
**Status:** Integrated (2026-09-17, `legacy-command-parity`). `.speed <0-500>` sets `movementSpeedMod` and `rotationSpeedMod` current values together (never max), atomically (both bounds checked before either is written — a rejected value leaves both stats untouched), publishing one `onStatUpdate`. **Real gap found and closed, not just a new command shipped:** `MOVEMENT_SPEED_MOD` had zero server-side read sites — `npc_movement_tick` stepped NPCs by the `CellEntity.move_speed` field only, so a faithful port would have set a stat nothing read, making this packet's own "prove it in the real tick" acceptance criterion unsatisfiable. Added `effective_move_speed()` to `cell/service/ticks/npc_movement.rs`, scaling the per-tick step by `movementSpeedMod.cur/100` (the client-side semantics `entities/defs/alias.xml` already documents, now also applied server-side authoritatively) — a no-op at the default `cur=100`, so all four pre-existing `npc_movement` tests kept their exact expected positions. `ROTATION_SPEED_MOD` has no equivalent consumer (the NPC tick snaps yaw instantly; no turn-rate integration exists) and is left client-applied-only as a documented, deliberate gap rather than inventing a movement feature. Two deliberate legacy deviations: rejects out-of-range input instead of legacy's silent clamp (matches native `gmSetHealth`'s precedent), and renders the feedback as an integer rather than legacy's bogus `%f`-formatted-int (`100.000000`). **Flagged, not fixed here:** a player's speed multiplier isn't consulted by `MovementValidator::DEFAULT_TOP_SPEED`, so `.speed 400` on a player emits `movement.speed_warning` events today (warn-only, harmless) — must be addressed before that layer is ever promoted to enforcement. Worknote/handoff: [worknotes/p47.md](worknotes/p47.md), [handoffs/p47.md](handoffs/p47.md). **Commands:** `.speed`. **Depends:** P01 only (split from P35 — no death/health lifecycle dependency). **Advisor:** movement-teleport-advisor.
**Entries:** [console stats](../../../crates/services/src/cell/console/stats.rs), [NPC movement tick](../../../crates/services/src/cell/service/ticks/npc_movement.rs), [legacy Entity](../../../deprecated/python/cell/commands/Entity.py).
