...
**Status:** Integrated (2026-09-17, `legacy-command-parity`, merge of `p02-info-facing-combatinfo` @ 9ba50c43). Reviewed by testing-validation-engineer (`.facing` geometry independently verified line-for-line against `SGWSpawnableEntity.py`; regression guards reproduced; fmt/clippy clean). UATPending — bundled into M1 once P03-P07 land. Worknote/handoff: [worknotes/p02.md](worknotes/p02.md), [handoffs/p02.md](handoffs/p02.md). **Known gaps carried forward, not design gates:** (1) `.combatinfo` implements 2 of legacy's 3 checks (no-template, no-ability-set); the weapon-presence check and the `ABILITY_TYPE_*` DD-count bucket have no equivalent concept in the current entity/ability model (no per-NPC-template weapon field, no ability-type taxonomy at all). This is a data-model absence, not an approval question, so it doesn't need a G-group — whichever future packet adds an ability-type taxonomy (if one ever is) should also close this gap in `query.rs::combat_info`. (2) `.facing`'s caller-yaw extraction (`query.rs::facing_angle`, reading `caller_dir.x`/`.z` via `atan2` as a Cartesian vector) inherits a pre-existing, cross-cutting ambiguity in `CellEntity.direction`'s own doc comment ("unit vector or yaw/pitch/roll encoded" — `entity_struct.rs:42`): real player movement packets deliver yaw/pitch/roll as compressed int8 bytes (`docs/reverse-engineering/findings/position-movement-wire-formats.md`, wire order yaw/pitch/roll, ~π/128 scale) that flow **unscaled** into `.direction` (`space_manager/entities.rs`'s `apply_client_position_update_at` → `update_entity_position`, raw `as f32` cast, no π/128 conversion), while GM-authored NPC paths (`console/entity.rs`'s `lookAt` setter) write a genuine Cartesian unit vector into the same field. `.facing`'s `atan2(x,z)` reading is consistent with 3 other existing call sites (`gm/query.rs::handle_show_rotation`, `console/spawn.rs`, parts of `npc_movement.rs`) but is likely wrong for a real player's live-updated direction — this affects the already-shipped native `.rotation` command too, not something P02 introduced. Not fixed here: resolving `CellEntity.direction`'s true semantics against the wire spec touches wire-parsing code well outside P02's owned files and needs its own investigation/design pass before any of these four call sites can be trusted. Flag as a candidate backlog item; not blocking P01/P02 integration. **Commands:** `.info`, `.facing`, `.combatinfo`. **Depends:** P01. **Advisor:** combat-systems-advisor.
...
**Status:** Approved (D15, 2026-09-17) — children enumerated below, each still individually Ready/BlockedDependency per its own line, not dispatchable as one lump. **Commands:** `.goto`, `.summon`, `.gotolocation`. **Depends:** P26. **Advisors:** movement-teleport-advisor, aoi-witness-broadcast, database-persistence.
