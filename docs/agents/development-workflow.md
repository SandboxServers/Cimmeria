...
- **One live test database per concurrent run.** Live-DB tests reload and mutate the database. Point each worktree at its own database on the bundled Postgres (`:5433`) through `DATABASE_URL`, and never reload a database another run is using. Starting the bundled Postgres and loading `db/database.sql` is documented in [`docs/architecture/integration-test-infra.md`](../architecture/integration-test-infra.md).
