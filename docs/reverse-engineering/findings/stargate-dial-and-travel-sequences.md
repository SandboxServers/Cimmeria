...
> **Date**: 2026-09-25 (research); implementation landed the same day once the owner lifted the session's usage restriction — see §5
> **Type**: Reference
...
> **Confidence**: HIGH for the wire-protocol structural claims (DHD dial is single-shot, chevron progress is never sent to the server); MEDIUM for the CrossGate per-instance Kismet matching mechanism; LOW/OPEN for exact Matinee/animation durations, which were not extracted this pass — the `GATE_DIAL_DURATION` and `CROSSING_CINEMATIC_HOLD` constants shipped in §5 are explicitly provisional placeholders, not measured values
> **Sources**: Ghidra decompilation of `SGW.exe` (see per-claim addresses below); `docs/reverse-engineering/findings/stargate-dhd-state-machine.md` (2026-05-13, session 5 — supplies several addresses corroborated independently here); `entities/defs/interfaces/GateTravel.def`; `crates/services/src/cell/gate_travel/`; `crates/services/src/cell/space_manager/gate_dial_state.rs` and its sibling `crossing_hold_state.rs`
...
**R1, R3 and R4 were implemented later the same day**, once the owner lifted the session's usage restriction. R2 was NOT implemented — it is a decision not to build something, not a code change. R5 remains open.

- **R1**: `GATE_DIAL_DURATION` is now `Duration::from_millis(100)` (`crates/services/src/cell/space_manager/gate_dial_state.rs`), with the doc comment recording the correction and the absence of a confirmed replacement duration. A regression test (`dial_opens_almost_immediately_not_after_a_multi_second_hold`) asserts the constant stays under 500ms.
- **R3**: `cell::gate_travel::on_stargate_passage` now sends `CellToBaseMsg::EntityMethodCall { method_index: ON_STARGATE_PASSAGE, .. }` to the crossing entity only, immediately after the `Stargate_CrossGate` sequence, carrying the 4-byte LE destination `addressId`. Byte-exact wire test:
  `gate_travel::tests::dial_timer::crossing_an_open_gate_sends_cross_gate_then_onstargatepassage_and_defers_travel`.
- **R4**: a new `PendingCrossing` deferred-tick state (`crossing_hold_state.rs`, a sibling of `gate_dial_state.rs` split out to keep each file under the project's line-count soft cap, mirroring `PendingGateDial`) is armed by `handle_stargate_region_entered` right after the crossing notifications, with the traveller's movement locked (`BSF_MovementLock`, via a small `set_crossing_movement_lock` helper in `sequences.rs` — kept separate from `ring_transport`'s equivalent rather than widening that module's `pub(super)` visibility for one external caller). A new `crossing_tick` (`gate_travel/tick.rs`), wired into the existing 100ms cell tick loop, drains the hold and runs the deferred `perform_gate_travel`. The lock is explicitly released if that deferred call fails (destination vanished, unrecoverable arrival, closed base channel) so a refused crossing does not leave the traveller stuck immobile; the hold itself is cancelled by `destroy_entity`/`disconnect_entity`, mirroring the dial timer's existing cleanup. `CROSSING_CINEMATIC_HOLD = Duration::from_millis(1_500)` — still explicitly provisional; no Matinee/Kismet duration was extracted this session (see open question 2 below).
- Two pre-existing tests asserted a **synchronous** `GateTravel` on crossing and needed updating to expire-and-drain the new hold first, or they would have passed vacuously regardless of the logic under test: `cell::cell_methods::player::world::tests::entering_a_stargate_region_with_an_open_dial_travels` and `cell::gate_travel::tests::arrival::crossing_into_an_unrecoverable_arrival_sends_no_transfer`.
- Full test list (all revert-proven): fake-clock timer tests for both durations (`gate_dial_state::tests`), the byte-exact wire test above, a hold-then-travel sequence test (`crossing_hold_elapsing_runs_the_deferred_travel`), a disconnect-during-hold test (`disconnect_during_the_crossing_hold_cancels_the_deferred_travel`), and a deferred-travel-failure test proving the lock releases (`deferred_travel_failure_clears_the_movement_lock`). `cimmeria-services` gate-related suite: 187 passed. `cargo +1.98.1 clippy -p cimmeria-services --all-targets -- -D warnings`: clean.
