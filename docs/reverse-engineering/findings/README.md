...
| `combat-formulas-status.md` | 2026-09 | **Start here.** Consolidated answer to "are the original combat formulas recoverable?" — per-question verified units/data vs unknowns (Accuracy/Defense, cover, weapon scaling, mitigation, Focus, stacking, AoE), the two Rust divergences from original enums, and what would close each gap. Summarises the two evidence docs below. |
| `combat-formulas-client-evidence.md` | 2026-09 | Evidence ledger for combat formulas (QR, cover, scaling, mitigation, Focus, stacking, AoE): what the client install proves and what is absent, with per-question gaps | HIGH (negatives reproducible) |
| `combat-formulas-game-data-evidence.md` | Shipped game data | Combat formulas — systematic survey of the seeded `resources` schema + original type declarations against the seven open formula questions (accuracy/defense, cover, weapon scaling, armor & damage types, focus, stacking, AoE/LOS). Establishes that the stat *taxonomy* shipped and the stat *values* did not; `effect_nvps` holds 17 original rows; `EF_DontUseQR` is the QR gate; cover nodes carry no numeric bonus | HIGH (data survey) — every claim tagged ORIGINAL-DATA / DESIGNER-TEXT / INFERENCE |
| `being-eye-heights.md` | 2026-09 (NA31) | Per-body-set eye heights for line of sight, measured from each body set's reference skeletal mesh bounds (the script pawn has one stock UE3 cylinder for every being); `ErrorStrings.pak` code 39 `CONDITION_FEEDBACK_LOS` is the only LoS feedback with text; `HEIGHT_LOS` is a cover tier, not an eye height | HIGH (bounds, units, codes) / MEDIUM (eye rule, presentation) |
...
| `stargate-dial-and-travel-sequences.md` | NA35 | Dial-to-kawoosh timing and gate-crossing cinematic — confirms the DHD dial is single-shot (no server-visible per-chevron progress) and the `Stargate_CrossGate` teardown race; the 4s `GATE_DIAL_DURATION` and the unsent `onStargatePassage` (client method 68) gaps this documents were fixed the same session (`cell::gate_travel`) | HIGH (wire-protocol structure) / LOW-OPEN (exact cinematic durations) |
