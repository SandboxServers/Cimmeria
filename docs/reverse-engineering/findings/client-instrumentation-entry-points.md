...
|---|---|---|---|
| `UObject::ProcessEvent` | `void(UFunction* Function, void* Parms, void* Result = NULL)` `__thiscall` | **RESOLVED 2026-07-25: `0x004e06a0`, vtable slot 58 (0x3A).** Body `0x004e06a0`–`0x004e0938`. Confirmed by the `UFunction::Func` dispatch fingerprint at `0x004e0899`: `MOV EAX,[EDI+0xa8]` / `PUSH ECX` (Result) / `LEA EDX,[EBP-0x3c]` (Frame) / `MOV ECX,EBX` (this) / `CALL EAX` — the indirect call through `UFunction::Func` this row previously named as the identifying criterion. Ghidra currently auto-names it `UTestIpDrv__vfunc_58` after the vtable it was recovered from; `UTestIpDrv` does not override the method, so slot 58 is the inherited `UObject` implementation. Agrees with [`../address-map.md`](../address-map.md). RTTI `.?AVUObject@@` @ `0x01dae610`; type_info @ `0x01dae608`; vtable @ `0x0180fe54` (~67 slots). Previously ruled out: slots 29, 30, 31, 32, 40. | **TRAP**: the only Ghidra function *named* `ProcessEvent` is `0x004513d6`, an import thunk resolving at runtime to wxWidgets `wxEvtHandler::ProcessEvent` (ordinal 12906) — unrelated to UE3. Do not hook it. Note the actual frame is `SUB ESP,0x30`, not the 140–160-byte alloca this row previously predicted. **MANDATORY for hook**: FName allowlist + thread-local re-entry guard. Sample 1/100 default, 1/1 in diag mode. |
