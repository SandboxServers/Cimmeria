...
not resolved by this session.** Either way, the geometry is real and should
be extracted; it's additional to, not a duplicate of, the persistent-level
`Model`.
...
## Appendix: `Terrain` recipe real-data validation (secondary task)
...
## Where the BSP geometry actually is (measured, whole-map)
