...
`docs/agents/rules-and-gotchas.md`, and `docs/agents/domain.md` ("When sources disagree").
...
- **The issue's premise is contradicted:** an issue is a claim, not evidence. If an advisor, a
  doc under `docs/protocol/` or `docs/architecture/`, or an RE finding contradicts what the issue
  says is wrong (a wire constant, a method index, a message layout, a formula), **do not implement
  the issue.** You cannot open Ghidra to settle it, so post the contradiction with both citations
  as an issue comment, add `ready-for-human`, unassign, and pick another issue. A well-tested PR
  built on a wrong premise costs the reviewer more than no PR.
- **Advisor disagreement on approach** (how to fix, not whether the premise holds): follow the
  issue and record the disagreement in the PR body. The reviewer settles it.
