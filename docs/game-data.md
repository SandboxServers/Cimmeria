...
- **Auction house / Black Market** — **not yet on `main`.** Implemented under [crates/services/src/base/black_market/](../crates/services/src/base/black_market/) (create, bid, cancel, search, expiry sweep, plus tests) and reachable in-world through the content engine's `open_black_market` action, but this is phase-1 work on the unmerged `feat/571-black-market-phase1` branch. Treat as in-flight, not shipped.
