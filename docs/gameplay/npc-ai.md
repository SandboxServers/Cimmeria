...
- **In cover:** on reaching the slot the NPC stops with zero velocity, gains Cover Stance (ability 1451, +100 `COVER_DEFENSE`), and fires from the slot without chasing. The stance is -1.0 QR on shots the cover faces and nothing on a flanking shot (NA32, [combat-system.md](combat-system.md#cover-in-the-qr-roll-na32)). An NPC holding its slot does not step back from a close target.
