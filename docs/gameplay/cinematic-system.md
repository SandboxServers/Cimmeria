...
Of the fourteen, the server emits exactly **two**: `Stargate_MakeGate` (6100) four seconds after a successful dial, and `Stargate_CrossGate` (6113) as the player enters the gate volume. Both are emitted by [`cell::gate_travel::sequences`](../../crates/services/src/cell/gate_travel/sequences.rs) to the dialer **and every witness of them**; see the [gate travel reference](gate-travel.md).
