...
| `spawner.npc_behaviour` | DEBUG | `cell::spawner::npcs::log_spawn_behaviour` | Resolved behaviour of each spawned NPC: `world`, `space_id`, `aggression`, `use_cover`, `is_stationary`, `move_speed`, `respawn_secs`, follow band, patrol / wander, `on_navmesh`, `ground_y`, `spawn_yaw_rad`, interaction flags, loot table, and (NA44) what it fights with: `ability_ids` (sorted), `event_set_ids` (each ability's event set in the same order; `0` = NULL or no loaded definition, so that attack plays no fire animation) and `weapon_visual` (first `WP` template component) |
