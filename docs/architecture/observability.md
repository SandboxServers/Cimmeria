...
| `npc_ai.path` | DEBUG | `cell::service::npc_ai::path_request::request_path` | `event = "request"`: every AI `find_path` (NA02). `state`, typed `status` (`ok` \| `partial` \| `no_start_poly` \| `no_end_poly` \| `no_corridor` \| `straighten_failed` \| `no_mesh`), `from` / `to`, `target_id`, `target_is_gm` (audit S14: a GM standing off-mesh fails every chase), `start_snap_dy`, `end_snap_dist`, `n_waypoints`, `max_leg_dy`, `end_to_target_dist`, `end_to_dest_dist`. A `partial` corridor is still walked, and also raises `npc_ai.path_fail reason=partial`. Counter `npc_path_requests_total` |
...
| `npc_ai.leash` | INFO / WARN / DEBUG | `cell::service::npc_ai::detectors::leash` | NA02. `event = "enter"` (INFO): Fighting → Leashing with `npc_to_spawn`, `target_to_spawn`, `leash_distance`, both positions, `nav_path_len`. `event = "snap_fallback"` (INFO; `arrived` for a follower, which is not moved): `snap_dist`, `stale_path_len` (the chase path the leash leaves behind, audit S4), `spawn_on_mesh`, `walk_secs = 0`, `path_ok = false`. `event = "loop"` (WARN, ≤ 1 / 60 s per NPC): 3 or more leash entries inside 60 s (S5), `leash_count`, `target_id`; counter `npc_leash_loop_total`. `event = "damage_ignored"` (DEBUG): threat landing on a Leashing NPC, which the leash discards (S12) |
| `npc_ai.aggro_scan` | DEBUG, sampled | `cell::service::npc_ai::detectors::aggro_scan` | NA02. `event = "candidate_rejected"` (≤ 1 / 10 s per NPC–player pair): `reason` (`same_faction` \| `not_player` \| `dead` today; NA13 adds the rest), `player_id`, `npc_to_target`, `dy`, `aggro_radius = "unbounded"`. `event = "no_candidates"` (≤ 1 / 30 s per NPC): `witness_count`, `rejected` |
| `npc_ai.idle` | DEBUG, ≤ 1 / 30 s per space | `cell::service::npc_ai::detectors::sweep` | `event = "unticked"`: how many Idle NPCs the AI tick skips (no aggression, patrol or wander), per world. Gauge `npc_ai_idle_unticked{world}` |
| `npc_ai.idle_parked` | INFO | `cell::service::npc_ai::detectors::idle_parked`, from `set_ai_state_on` | NA02. An NPC changed to Idle more than 2 u from spawn and will not be ticked again (S6 + A1): `from`, `reason` (the transition reason), `npc_to_spawn`, position. Counter `npc_idle_parked_total{world,reason}`. Should read zero after NA12 |
| `npc_ai` | WARN | `cell::service::npc_ai::detectors::sweep` | NA02, per ticked NPC after its handler. `event = "npc_off_mesh"` (≤ 1 / 30 s): `gate`, `horizontal_dist`, `dy`, `last_move_source` (`path` \| `fallback` \| `leash` \| `backup` \| `content` \| `spawn`); counter `npc_off_mesh_total{world,gate}`. `event = "stuck"` (≤ 1 / 15 s): Fighting and chasing with a path, and `npc_to_target` has not shrunk by 0.5 over 3 AI ticks — `npc_to_target_history`, `nav_path_len`, `los`, `next_wp`; counter `npc_stuck_total` |
| `npc_ai` | DEBUG | `cell::service::npc_ai::fight_cover::route_via_cover` | NA02, `decision_outcome = "no_cover"` (a log field, not the tick's terminal outcome): replaces the silent `NoCover => {}` arm (audit C7). `reason` (`use_cover_false` \| `stationary` \| `no_candidate_in_radius` \| `reserve_lost` \| `in_range_no_better_slot` \| `index_miss`), `candidates_scanned`, `reserved_skipped`, `search_radius`, `cover_nodes_loaded` |
| `movement.npc` | WARN | `cell::service::npc_ai::detectors::movement` | NA02. `event = "stale_velocity"` (≤ 1 / 10 s per NPC): a non-zero velocity with no displacement for 3 movement ticks — the running-in-place detector (S1). `path_state` = `empty` (the path was cleared mid-leg; this is also the telemetry plan's `animating_without_path`, re-based on velocity because the client animates from velocity alone) or `stalled`; counter `npc_stale_velocity_total`. `event = "ground_deviation"` (≤ 1 / 5 s per NPC): a step or waypoint snap more than 0.3 from the storey-aware floor (NA01's `get_height_near`) on a meshed world — `dir` (`up` \| `down` \| `unknown` when no floor is within jump height), `ground_y`, `dy`, `y_source` (`lerp` \| `waypoint`), `leg_len`, `leg_dy`, `wp_*`; counter `npc_ground_deviation_total{world,dir}` |
| `threat` | WARN, ≤ 1 / 30 s per player–NPC pair | `cell::service::npc_ai::detectors::threat` | NA02, `event = "cleared_without_exit"`: an NPC cleared its threat list (`reason` = `threat_empty` \| `leash_out` \| `leash_complete`) while a player still lists it in `threatened_mobs` (S7), so the player stays in combat. Counter `npc_threat_cleared_without_exit_total{world,reason}` |
...
| `cover.coverage` | INFO, WARN when unusable | `cell::cover::coverage::log_space_coverage` | NA02, `event = "space_summary"`, one row per space once the cover index loads: `nodes_total`, `nodes_in_bounds`, `nodes_on_mesh` (the `find_path` start box), `sets_in_bounds`, `sets_origin_centred`. WARN `reason` = `no_cover_on_mesh`, or `prefab_local_coordinates` when most sets in bounds straddle the world origin (audit C1). The second test exists because today's seed puts ~3,000 nodes on Cellblock's `y ≈ 0.2` ground plane by coincidence, so "nothing on the mesh" never fires |
...
| `npc_path_fail_total` | `world` (~24), `state` (5), `reason` (7) | Same discipline: counted every AI tick, logged ≤ 1 / 5 s per NPC |
...
| `npc_stale_velocity_total`, `npc_stuck_total`, `npc_leash_loop_total` | `world` (~24) | NA02 detectors: every occurrence, including throttled ones |
| `npc_ground_deviation_total` | `world` (~24), `dir` (3) | NA02 |
| `npc_off_mesh_total`, `npc_spawn_off_mesh_total` | `world` (~24), `gate` (≤ 5) | NA02 |
| `npc_idle_parked_total` | `world` (~24), `reason` (18) | NA02 |
| `npc_threat_cleared_without_exit_total` | `world` (~24), `reason` (3) | NA02, one per player still listing the NPC |
