...
| `npc_ai` | DEBUG | `cell::service::npc_ai::fight_cover::route_via_cover` | NA02, `decision_outcome = "no_cover"` (a log field, not the tick's terminal outcome): replaces the silent `NoCover => {}` arm (audit C7). `reason` (`use_cover_false` \| `stationary` \| `no_candidate_in_radius` \| `reserve_lost` \| `in_range_no_better_slot` \| `no_world` \| `index_miss`), `candidates_scanned`, `reserved_skipped`, `search_radius`, `cover_nodes_loaded` |
