...
> The 60/316 propID thresholds documented above are inherited from BigWorld 2.0.1 source (`property_change.hpp`). SGW.exe does not contain a wire-format propID decoder at all — exhaustive byte-pattern search (audit Appendix D.3) confirms the `0x3C`/`0x3D` comparison instructions are absent from client code. The thresholds are therefore demonstrably a server-side-only concern that cannot be verified or falsified from any SGW client binary. Server implementers MUST cross-check these thresholds against the actual BigWorld 2.0 server binary or a live wire-encoded capture taken upstream of the UE3 bridge (the propID arrives at `FNetworkPropertyChange__vfunc_0` already decoded; capturing the bridge input does not see the encoded form). A server that assumes incorrect thresholds will produce property updates the client misinterprets — silently writing the wrong propID slot, just like the §1.1 "get the table wrong by one slot" failure mode but at the header layer.
