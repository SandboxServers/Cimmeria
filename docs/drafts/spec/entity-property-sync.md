...
**Status: MEDIUM confidence.** The runtime property delta rides on top of the `updateEntity` system message (msg_id `0x0A`, `WORD_LENGTH`-framed) — its envelope is the Mercury bundle and its msg_id slot lives in the client descriptor table catalogued in [`spec.protocol.mercury-wire-format` §2.5](mercury-wire-format.md#25-client-descriptor-table--system-message-handlers) (msg_id 0x0A, `word-prefix`, "Per-entity property delta") and in [`spec.protocol.message-catalog`](../../protocol/message-catalog.md). Mercury does not pin a byte-by-byte payload subsection for `updateEntity`. **On the client side, the payload is a typed UE FArchive envelope, not a BigWorld propID-prefixed wire stream**: the bridge `FUN_01560ad0 @ ghidra://SGW.exe@0x01560ad0` reads `[u32 length][u32 type tag]` from the head of the body, validates `length == remaining`, and dispatches on the type tag (case 1 = `FNetworkPropertyChange::vfunc_0`, which then reads the propID as a `uint32_t` from the FArchive — see the dispatch chain below). The BigWorld 0x3C/0x3D wire-prefix scheme described later in this section is decoded **upstream** of this bridge by the BigWorld network layer and is only visible to a capture taken on that upstream wire, not to one taken at the client. The 60/316 thresholds remain server-encoder-side only; the byte-pattern absence finding (Appendix D.3) confirms they have no client-side decoder presence.
...
The property-change *wire encoder* resides on the BigWorld server (BaseApp/CellApp), not in SGW.exe. Its output is decoded by the BigWorld client message handler before being handed to the Unreal replication layer. The full client-side dispatch chain (audit Appendix E) is:

```text
updateEntity_Handler @ ghidra://SGW.exe@0x00dd62c0      (Mercury msg_id 0x0A handler)
  → FListenHelper::vtable[5] = FUN_01561140 @ 0x01561140  (singleton guard; forwards (pData, nLen))
    → FUN_01560ad0 @ 0x01560ad0                          (BigWorld→UE bridge; 4-byte type tag switch)
      case 1 → FNetworkPropertyChange__vfunc_0 @ 0x015652d0  (property delta — this section)
...
`updateEntity` is therefore a typed envelope, not a property-only channel — property delta is the case-1 sub-message. `FUN_01560ad0` reads 4 bytes (payload length) + 4 bytes (type tag), validates `payload_len == read_len`, and dispatches on the type tag. The `FListenHelper` singleton is allocated by `FUN_0155f9b0 @ ghidra://SGW.exe@0x0155f9b0`, stored at `DAT_01ef11fd8`, with vtable at `ghidra://SGW.exe@0x01b14d4c`. The intermediary `FListenHelper::vtable[5]` step performs a one-level null-check on the bridge sub-object at `this+0x28` and tail-calls `FUN_01560ad0`.
...
**These threshold values (60, 316) cannot be confirmed from SGW.exe alone** — the encoder is server-side. An exhaustive byte-pattern search of SGW.exe's code (audit Appendix D.3) found **zero** instances of `0x3C`/`0x3D` threshold-comparison instructions in any form — neither `CMP AL, 0x3C`, `CMP EAX, 0x3C`, `MOVZX + CMP`, nor any `SUB EAX, 60` decoder pattern appears in code outside unrelated XML/CSS/Huffman parsers. The client demonstrably contains no BigWorld wire-format propID decoder; by the time a propID reaches `FNetworkPropertyChange__vfunc_0 @ 0x015652d0` it is already a fully-decoded 32-bit integer read directly from the UE3 FArchive at `this+0x2c`. BigWorld's wire-encoded propID decoding happens upstream of the UE3 bridge — the server emits the encoded prefix, BigWorld's network layer decodes it before the property delta enters `FUN_01560ad0`. The only client-side confirmation is that `EntityDescription_GetClientPropertyByIndex @ ghidra://SGW.exe@0x01590d80` maps a `nClientPropIndex` to a DataDescription. In stock BigWorld this is an index into the `+0x70/+0x74` pointer array; in SGW it indexes the main DataDescription array at `+0x5c/+0x60` (per §1.2's source-doc override, since the `+0x70/+0x74` array is empty in practice). Either way, the wire propID *is* this index. The 0x3C/0x3D threshold values must be verified against the actual server binary or BigWorld 2.0.1 source — see the implementer warning callout above.

> [!WARNING] **Implementer warning — OQ-1 in §1.15. The encoded propID lives only on the server-side wire.**
> The 60/316 propID thresholds documented above are inherited from BigWorld 2.0.1 source (`property_change.hpp`). SGW.exe does not contain a wire-format propID decoder at all — exhaustive byte-pattern search (audit Appendix D.3) confirms the `0x3C`/`0x3D` comparison instructions are absent from client code. The thresholds are demonstrably a **server-encoder-side** concern. Two implications for implementers:
>
> 1. **Server side**: emit the prefix-encoded form. A server that assumes incorrect thresholds will produce property updates the client misinterprets — silently writing the wrong propID slot, just like the §1.1 "get the table wrong by one slot" failure mode but at the header layer.
> 2. **Wire-capture side**: the encoded form is **not visible** to a pcap taken at the client. By the time bytes hit `FNetworkPropertyChange__vfunc_0` the propID is already a fully decoded `uint32_t` read from the UE FArchive (after `FUN_01560ad0` has stripped the envelope's `[u32 length][u32 type tag]`). Any wire-format probe of the 0x3C/0x3D scheme must capture the encoded form **upstream** of the BigWorld→UE bridge, e.g. at the BigWorld network layer on the server or inside the BigWorld decoder before it hands off to the UE bridge. A client-pcap capture of `updateEntity` (msg_id `0x0A`) shows the envelope, not the prefix.
...
**Wire-capture attempt (V4, audit Appendix C.5) — superseded by OQ-1 closure.** A `sessions/2026-05-16_08-21.pcap` capture covering character-select and the first seconds of world-entry observed **zero** `updateEntity` (msg_id `0x0A`) messages — no in-world stat updates, no inventory churn, no ability use occurred during the window. The earlier framing of V4 ("scan the first byte of the `updateEntity` payload for 0x3C/0x3D propID prefixes") was a methodology error: from the client side, the first 8 bytes of the body are the UE FArchive envelope (`[u32 length][u32 type tag]`), not a BigWorld propID prefix. The byte-pattern-absence finding in Appendix D.3 makes a client-binary capture moot for the 0x3C/0x3D thresholds — even a long, busy capture would only confirm what type-tag and decoded propID values appeared, not whether the server emitted the upstream prefix correctly. Path to closure of the threshold question therefore requires either (a) instrumenting the BigWorld server's network-encoder side, or (b) an x64dbg breakpoint at the bridge's input (the BigWorld decoder, not `FUN_01560ad0`) capturing the wire-encoded form before decoding. Tracked as OQ-1 in §1.15. The `entity_property_sync_resolver.py` tool's V4 validator now decodes the FArchive envelope and reports the decoded propID distribution (so reviewers can see which threshold-encoding range the server *would* have used) rather than searching for the absent prefix bytes.
...
| Property-change propID threshold values (60/316) | BigWorld 2.0.1 `property_change.hpp` (not in SGW.exe; server-side only) | Old RE doc §6; **client-side decoder absent — confirmed by exhaustive byte-pattern search, audit Appendix D.3** |
| `updateEntity` (msg_id 0x0A) dispatch chain: `updateEntity_Handler → FListenHelper::vtable[5] → FUN_01560ad0 → case 1 = FNetworkPropertyChange__vfunc_0` | `updateEntity_Handler @ ghidra://SGW.exe@0x00dd62c0` (full disassembly, audit Appendix E.1); `FListenHelper::vtable[5] = FUN_01561140 @ ghidra://SGW.exe@0x01561140`; bridge `FUN_01560ad0 @ ghidra://SGW.exe@0x01560ad0` 4-byte type-tag switch | Audit Appendix E.1–E.3; `FListenHelper` singleton at `DAT_01ef11fd8`, vtable at `ghidra://SGW.exe@0x01b14d4c` |
...
**OQ-1: CLOSED — thresholds are server-side only.**
Exhaustive byte-pattern search of SGW.exe code (audit Appendix D.3) found zero instances of `0x3C`/`0x3D` threshold-comparison instructions. The BigWorld wire propID encoding is decoded upstream of `FNetworkPropertyChange__vfunc_0 @ 0x015652d0`, and arrives at the UE3 FArchive layer as fully-decoded `uint32_t` values. The 60/316 thresholds remain BW-source-only and cannot be verified or falsified from any SGW client binary — they are server-side encoding constants. Server implementers must cross-check against the actual BigWorld 2.0 server binary or a live wire-encoded capture taken upstream of the UE3 bridge. The implementer warning in §1.8 and R6 remain in force: a server emitting wrong thresholds will silently mis-target propID slots on the client. The prior wire-capture attempt at `sessions/2026-05-16_08-21.pcap` (audit Appendix C.5) yielded no `updateEntity` traffic and is now superseded — the byte-pattern absence finding makes a client-binary capture moot.
...
**OQ-2: RESOLVED — the dual-name-field hypothesis was wrong; the slots are vestigial BigWorld infrastructure.**
Closed by audit Appendix F.2 + F.2.1 + F.2.2. The XML element name lives at `+0x04`, not at `+0x24` or `+0x40`. The 0x110-byte DataDescription has two distinct layouts at the same size:

- **Parse-time form** (`DataDescription_Constructor @ ghidra://SGW.exe@0x01591fb0`, `DataDescription_ParseFlags @ ghidra://SGW.exe@0x015974a0`): `+0x04` is a `StdStringMSVC` holding the XML element name (from `pSection->getName()`). `+0x24` is repurposed as a `SmartPointer<DataSection>` for the `<Default>` XML child node — NOT a string. `+0x40` is a `StdStringMSVC` slot zero-initialized by the constructor and **never written** anywhere in the SGW parse path. `FUN_0158f260 @ ghidra://SGW.exe@0x0158f260` is a partial-struct copier for `+0x00..+0x3c` only and does not touch `+0x40`.
- **Runtime form** (read by `EntityDescription_FindAndWritePropertyByName @ ghidra://SGW.exe@0x0158e780`): both `+0x24` and `+0x40` are read as `StdStringMSVC` for the consistency-gate comparison, but the runtime values are exactly what the parse left in place: `+0x24` is the SmartPointer bytes (length-field reads as 0); `+0x40` is the zero-length empty string from the constructor.

The comparison in `EntityDescription_FindAndWritePropertyByName` reduces to `compare(any, any, min(0, 0)) = 0` followed by `0 == 0` — a **tautological dead check** that every DataDescription passes unconditionally. The earlier F.2.1 framing of this as "a name-consistency gate between server-symbolic and client-alias variants" is corrected by F.2.2: the slots were intended for a BigWorld feature (alias / display-name) that SGW never wired up. The Ghidra constructor annotation that labelled `+0x24` "type name" and `+0x40` "alias name" is wrong; both slots are dead in SGW.

This closes both OQ-2 and the OQ-2-bis sub-question. Wire-irrelevant either way — the property name on the wire is the XML element name at `+0x04`, not anything at `+0x24` / `+0x40`.
...
**F1 — Property update with out-of-range propID — reframed as server-encoder concern.** The client does not perform wire-level propID bounds-checking. Per §1.8 and OQ-1, SGW.exe contains no BigWorld wire-format propID decoder; propIDs arrive at `FNetworkPropertyChange__vfunc_0 @ ghidra://SGW.exe@0x015652d0` as already-decoded 32-bit integers from the UE3 FArchive at `this+0x2c`. "What happens if the server encodes a propID exceeding the entity's DataDescription array bounds?" is therefore a server-encoder question, not a client-decoder question — by the time the value reaches the client's `FArchive` consumption layer it is whatever the upstream BigWorld network code handed it. No client-side bounds check on the decoded propID was found in either `FNetworkPropertyChange__vfunc_0` or `FUN_01560ad0 @ 0x01560ad0` (the bridge does a `payload_len == read_len` check, not a propID range check). **Status: NOT-DETERMINABLE from client binary.** Resolving the on-the-wire failure mode requires server-side instrumentation or a BW 2.0 server-binary trace. (Audit B.4 / G13 / D.4.)
...
**R6 — On the server-side wire (BigWorld→BigWorld, upstream of the UE bridge), encode property updates with the propID-prefix scheme `propId < 60 → 1 byte; 60..315 → [0x3C, propId-60]; 316+ → [0x3D, propId-316]`.** The thresholds are inherited from BigWorld 2.0.1 `property_change.hpp`. SGW.exe contains no client-side decoder for this scheme (OQ-1 CLOSED, §1.15 — Appendix D.3 byte-pattern absence), so it cannot be falsified from a client-side pcap; verification requires either the actual BigWorld 2.0 server binary or a capture taken upstream of the UE bridge. **The downstream client-visible payload is a UE FArchive envelope (`[u32 length][u32 type tag]`) with the propID delivered as a `uint32_t` after type-tag 1** — implementers MUST NOT emit the 0x3C/0x3D prefix bytes into the UE-facing message; that path expects an already-decoded propID. **Citation**: §1.8 + §1.15 OQ-1.
...
| 3 | `0x03` | `SGWGmPlayer` | no | GM tooling player variant. |
