...
[Cimmeria server-side note: `Channel::receive_parsed` (`crates/mercury/src/channel/rx_order.rs`) is this gate. The server runs it on every client packet, anchored at seq 0: in the `castle_cellblock_head` capture the client's first reliable packet is seq 0, flags `0x58`. The loopback/wireclient harness runs it too. Unlike the client, it does not ack a packet beyond the window, so the sender retransmits it.]
