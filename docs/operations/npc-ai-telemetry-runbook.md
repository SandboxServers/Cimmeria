...
- **`cimmeria.deploy_env = 'colo'` returns nothing until the colo compose is re-applied.** At UAT-1 (2026-09-25) colo rows still carried `cimmeria.deploy_env = 'dev'` and a container id as `host.name`: Watchtower pulls the new image but does not re-apply `docker/compose*.yml`, so the environment the compose sets never reaches the container. Until the owner re-applies it, scope to the colo with `service.version = '<sha>'` (the build you deployed) or the colo's `host.name` instead.
...
| Why is this NPC holding fire, or firing with `los=blocked`? | View **NPC AI — Why is this NPC holding fire?** plus `AND npc_id = N` and a short time range | On `npc_ai.tick` rows, `los` is the navmesh verdict and `los_policy` is the rule that acted on it. `los=blocked los_policy=stationary_relaxed` is a stationary NPC firing across furniture the navmesh cuts out, which is expected (D-NA11). `stationary_other_storey` is a stationary NPC holding because the target is more than 4 u above or below it. `cover_peek` is a mobile NPC at its cover slot, whose line was checked from the slot's peek point past the prop (NA23, D-NA12): with `los=clear` it fires, with `los=blocked` it holds fire (`decision_outcome=cover_no_shot`) and gives the slot up after 3 s (`cover_released_no_shot`). On builds before NA23 the label was `in_cover_slot`, which fired whatever the verdict: UAT-1 found every one of its 31 rows read `los=blocked`, and one of those shots went through two walls. `strict` with `blocked` is a mobile NPC walking toward its target. The sampled `npc_ai.los` rows give the ray endpoints and `hit_xyz`; `origin` says whether the ray started at the NPC (`npc`) or at its peek point (`cover_peek`, `cover_no_peek`) |
