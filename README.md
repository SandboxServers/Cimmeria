...
*Generated by `tools/crate-graph/crate_graph.py` from `cargo metadata`: 29 workspace crates, 41 direct dependency edges (25 shown; an edge already implied by a longer path is omitted). Dev-dependencies are not drawn. Regenerate with `python tools/crate-graph/crate_graph.py`; CI fails when this block is stale.*
