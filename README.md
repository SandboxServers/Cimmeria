...
*Generated by `tools/crate-graph/crate_graph.py` from `cargo metadata`: 32 workspace crates, 52 direct dependency edges (31 shown; an edge already implied by a longer path is omitted). Dev-dependencies are not drawn. Regenerate with `python tools/crate-graph/crate_graph.py`; CI fails when this block is stale.*
