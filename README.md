...
*Generated by `tools/crate-graph/crate_graph.py` from `cargo metadata`: 31 workspace crates, 46 direct dependency edges (28 shown; an edge already implied by a longer path is omitted). Dev-dependencies are not drawn. Regenerate with `python tools/crate-graph/crate_graph.py`; CI fails when this block is stale.*
