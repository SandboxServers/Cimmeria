...
*Generated by `tools/crate-graph/crate_graph.py` from `cargo metadata`: 30 workspace crates, 44 direct dependency edges (27 shown; an edge already implied by a longer path is omitted). Dev-dependencies are not drawn. Regenerate with `python tools/crate-graph/crate_graph.py`; CI fails when this block is stale.*
